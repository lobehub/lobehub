...
Given('用户进入 Chinna 对话页面', { timeout: 30_000 }, async function (this: CustomWorld) {
  ...
  console.log('   📍 Step: 查找 Chinna...');
  // Find and click on "Chinna" agent in the sidebar/home
  const lobeAIAgent = this.page.locator('text=Chinna').first();
  await expect(lobeAIAgent).toBeVisible({ timeout: WAIT_TIMEOUT });

  console.log('   📍 Step: 点击 Chinna...');
...
Given('用户已发送消息 {string}', async function (this: CustomWorld, message: string) {
  ...
  // Assistant messages are left-aligned .message-wrapper elements that contain "Chinna" title
  ...
  const assistantMessage = this.page.locator('.message-wrapper').filter({
    has: this.page.locator('text=Chinna'),
  });
...
Then('回复内容应该可见', async function (this: CustomWorld) {
  const assistantMessage = this.page.locator('.message-wrapper').filter({
    has: this.page.locator('.message-header', { hasText: /Chinna|AI/ }),
  });
