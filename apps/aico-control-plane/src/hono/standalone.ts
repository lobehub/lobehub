...
import { ensureBootstrapAdmin } from './operatorAuth';
...
const resolvePort = () => {
  const parsed = Number.parseInt(
    process.env.AICO_CONTROL_PLANE_PORT ?? process.env.HONO_PORT ?? process.env.PORT ?? '',
    10,
  );
...
const server = createServer((request, response) => {
  void (async () => {
    try {
      ...
      console.error('Aico control plane request failed:', error);
...
const startServer = async () => {
  ...
  standaloneGlobal.__aicoControlPlaneServer = server;

  process.title = `aico-control-plane-${port}`;
  server.listen(port, host, () => {
    console.info(`Aico control plane ready at http://${host}:${port}`);
  });
};

void startServer().catch((error) => {
  console.error('Failed to start Aico control plane:', error);
  process.exitCode = 1;
});
