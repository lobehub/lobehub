...
import type { SearchSyncFailure, SearchSyncWork } from '@/database/repositories/searchSyncOutbox';

import { ElasticsearchSearchRequestError } from '../searchBackend/elasticsearch';
import { SearchSyncService } from './service';

const work = (documentId: string, revision: number = 1): SearchSyncWork => ({
  documentId,
  entity: 'agents',
  leaseToken: `${revision}`,
  revision,
...
const createHarness = (works: SearchSyncWork[], sources: Map<string, Record<string, unknown>>) => {
  const builder = {
  ...
  const outbox = {
    acknowledgeMany: vi.fn(async (items: SearchSyncWork[]) => items),
    claim: vi.fn(async () => works),
    hasActionableWork: vi.fn(async () => false),
    hasDeadLetters: vi.fn(async () => false),
    markFailures: vi.fn(
      async (failures: SearchSyncFailure[]) =>
        failures.filter((failure) => failure.permanent).length,
    ),
    releaseMany: vi.fn(async (_items: SearchSyncWork[]) => undefined),
  };
...
describe('SearchSyncService', () => {
  ...
  it('writes a soft tombstone when the PostgreSQL row no longer exists', async () => {
    ...
    expect(harness.client.bulk).toHaveBeenCalledWith(
      expect.stringContaining('{"id":"deleted-agent","search_sync_deleted":true}'),
    );
  ...
  it.each([401, 403, 408, 413, 429, 503])(
    ...
    async (status) => {
      ...
      harness.client.bulk.mockRejectedValue(
        new ElasticsearchSearchRequestError('bulk failed', status),
      );
      const service = new SearchSyncService(
        harness.builder as never,
        harness.outbox as never,
  ...
  it('dead-letters an entity that the projection builder does not support', async () => {
    const item = { ...work('unknown'), entity: 'removedEntity' } as unknown as SearchSyncWork;
  ...
  it('releases every unsettled lease when acknowledgement fails without hiding the main error', async () => {
    ...
    expect(consoleError).toHaveBeenCalledWith(
      'Failed to release unsettled search sync leases',
      releaseError,
    );
