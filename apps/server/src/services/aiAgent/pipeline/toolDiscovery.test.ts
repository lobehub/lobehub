...
const buildGate = (toolGrants: AgentShareToolGrant[]): AgentShareGate => ({
  agentId: 'agent-1',
  shareConfig: {
...
describe('applyShareVisitorSandboxOverride', () => {
  ...
  it('leaves the plan untouched when the share does not grant lobe-cloud-sandbox', () => {
    const gate = buildGate([{ identifier: 'web-search' }]);
  ...
  it('forces the plan to sandbox when the share grants lobe-cloud-sandbox and the plan degraded to none', () => {
    const gate = buildGate([{ identifier: CloudSandboxManifest.identifier }]);

    expect(applyShareVisitorSandboxOverride(nonePlan, gate, undefined)).toEqual(sandboxPlan);
  });

  it('recognizes an `apis`-scoped grant as a grant of the whole identifier', () => {
    const gate = buildGate([{ apis: ['runCommand'], identifier: CloudSandboxManifest.identifier }]);
