...
vi.mock('./observability', () => ({
  recordElasticsearchSearchRequest: mocks.recordSearchRequest,
}));
...
describe('ElasticsearchHttpClient', () => {
  it('returns no write targets without making requests when no aliases are provided', async () => {
    ...
    const client = new ElasticsearchHttpClient({
      apiKey: 'test-api-key',
      url: 'https://search.example.com',
    });

    await expect(client.getSearchSyncWriteTargets([])).resolves.toEqual({});
  ...
  it('verifies writable aliases and their soft-delete mappings before synchronization', async () => {
      ...
      .mockResolvedValueOnce(
        Response.json({
          'lobehub-agents-v2': {
            mappings: {
              search_sync_deleted: {
                full_name: 'search_sync_deleted',
                mapping: { search_sync_deleted: { type: 'boolean' } },
              },
            },
          },
          'lobehub-topics-v2': {
    ...
    await expect(
      client.assertSearchSyncAliases(['lobehub-agents', 'lobehub-topics']),
    ...
    expect(fetchMock.mock.calls.map(([url]) => url.toString())).toEqual([
      ...
      'https://search.example.com/lobehub-agents-v2,lobehub-topics-v2/_mapping/field/search_sync_deleted',
    ]);
  });
  ...
  it('returns writable physical indices in stable alias order', async () => {
    ...
    const writeTargets = await client.getSearchSyncWriteTargets([
      'lobehub-topics',
      'lobehub-agents',
    ]);
  ...
  it('returns stable runtime identities for indices from one reindex run', async () => {
        ...
        Response.json({
          'lobehub-agents-v2': {
            mappings: {
              ...
              properties: {
                id: { type: 'keyword' },
                search_sync_deleted: { type: 'boolean' },
              },
            },
            settings: {
              index: {
    ...
    const identities = await client.getSearchSyncIndexIdentities([
      'lobehub-topics',
      'lobehub-agents',
    ]);
  ...
  it('rejects a runtime identity response with missing reindex metadata', async () => {
      ...
      vi
        ...
        .mockResolvedValueOnce(
          Response.json({
            'lobehub-agents-v2': {
              mappings: {
                properties: { search_sync_deleted: { type: 'boolean' } },
    ...
    const request = client.getSearchSyncIndexIdentities(['lobehub-agents']);
  ...
  it('rejects aliases that point to indices from different reindex runs', async () => {
    ...
    await expect(
      client.getSearchSyncIndexIdentities(['lobehub-agents', 'lobehub-topics']),
    ).rejects.toThrow('Elasticsearch search sync aliases do not share one reindex run identity');
  });
  ...
  it('selects the unique explicit write index for a multi-target alias', async () => {
    ...
    await expect(client.getSearchSyncWriteTargets(['lobehub-agents'])).resolves.toEqual({
      'lobehub-agents': 'lobehub-agents-v2',
    });
  });
  ...
  it('rejects a multi-target alias without an explicit write index', async () => {
    ...
    await expect(client.getSearchSyncWriteTargets(['lobehub-agents'])).rejects.toThrow(
      'Elasticsearch search sync destination is not a writable alias: lobehub-agents',
    );
  ...
  it('rejects an alias whose write index lacks the soft-delete mapping', async () => {
    ...
    await expect(client.getSearchSyncWriteTargets(['lobehub-agents'])).rejects.toThrow(
      'Elasticsearch search sync alias lacks a boolean search_sync_deleted mapping: lobehub-agents',
    );
  });
});
