...
LobeHub **opens** DM handling for QQ by default. Adjust **DM Policy** in **Advanced Settings** if you want to restrict 1:1 access:

- **Open (default)** — Any QQ user who shares context with the bot can DM it.
- **Allowlist** — Only the QQ users you list in **Allowed User IDs** can DM the bot. Use the platform `tiny_id` from the OpenAPI event payload — the visible QQ number is not always the same as the platform ID.
...
| **Allowed User IDs** | No       | Comma- or whitespace-separated QQ `tiny_id` values. Only used when DM Policy is Allowlist |
