...
Slack 的私信限定在 workspace 内，因此 LobeHub 默认对 Slack **开放** 私信 — workspace 内任何成员都可以私信机器人。如需收紧，可以在 **高级设置** 调整 **私信策略 (DM Policy)**：

- **开放 (Open)（默认）** — workspace 内任何成员都可以私信机器人。
- **白名单 (Allowlist)** — 只有列在 **允许的用户 ID** 里的 Slack 成员可以私信。打开用户资料 → 点击 `⋮` → **复制成员 ID**（以 `U` 开头）。
- **禁用 (Disabled)** — 机器人忽略所有私信，只在频道里被 `@提及` 时回复。

私信策略只影响私信，频道里的 `@提及` 不受影响。跨平台细节见 [渠道概览](/docs/usage/channels/overview#direct-message-policy)。
...
| **允许的用户 ID**   | 否             | 逗号或空格分隔的 Slack 成员 ID。仅在私信策略为白名单时生效               |
