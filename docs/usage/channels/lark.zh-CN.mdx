...
Lark 应用绑定在租户内，因此 LobeHub 默认对 Lark **开放** 私信 — 租户内任何能看到机器人的成员都可以私信。如需收紧，可以在 **高级设置** 调整 **私信策略 (DM Policy)**：

- **开放 (Open)（默认）** — 租户内任何成员都可以私信机器人。
- **白名单 (Allowlist)** — 只有列在 **允许的用户 ID** 里的 Lark 用户可以私信。使用事件 payload 里的 `open_id`，或 Lark 开发者后台显示的 **User ID**。
- **禁用 (Disabled)** — 机器人忽略所有私信，只在群聊里被 `@提及` 时回复。

私信策略只影响私信，群聊里的 `@提及` 不受影响。跨平台细节见 [渠道概览](/docs/usage/channels/overview#direct-message-policy)。
...
| **允许的用户 ID**               | 否    | 逗号或空格分隔的 Lark `open_id`。仅在私信策略为白名单时生效            |
