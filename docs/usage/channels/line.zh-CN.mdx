...
- 一个 [LINE Business ID](https://account.line.biz/login)（任意 LINE 账号都能登录）
- [LINE Developers Console](https://developers.line.biz/console/) 中的 **Provider**（首次接入时点击 "Create a new provider" 创建一个）
...
<Steps>
  ...
  登录 [developers.line.biz/console](https://developers.line.biz/console/)，选择已有的 Provider，或者点击 **Create a new provider** 起一个名字（例如 "LobeHub"）。

  ### 创建 Messaging API channel

  在 Provider 内点击 **Create a new channel** → **Messaging API**。填写名称、描述、分类、地区，提交。

  ### 记录 channel 的关键标识

  打开 **Basic settings** 选项卡，复制以下值供第四步使用：
  ...
  - **Bot info** 区域中的 **destination user ID**（以 `U` 开头，共 33 位）。这是 LobeHub 需要的 **Destination User ID**。
</Steps>
...
<Steps>
  ...
  | 字段                       | 值                                      |
  | ------------------------ | -------------------------------------- |
  | **Destination User ID**  | Messaging API → Bot info 中的 `U…` 用户 ID |
...
| 字段                       | 是否必需 | 描述                                                                               |
| ------------------------ | ---- | -------------------------------------------------------------------------------- |
| **Destination User ID**  | 是    | 机器人的用户 ID（`U` + 32 位十六进制），来自 **Messaging API → Bot info**。同时作为机器人标识与 Webhook 路径。 |
...
- **`Channel access token belongs to bot Uxxx, not Uyyy`。** Destination User ID 与 Token 不匹配。打开 **Messaging API → Bot info**，把准确的 `U…` 值填进 Destination User ID 字段。
