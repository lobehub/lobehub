...
<Steps>
  ...
  > **注意：** 同一选项卡里的 **"Your user ID"** 是**你账号自己的** LINE user ID，**不是** bot 的 destination user ID。两者格式完全一样（都以 `U` 开头共 33 位），但 LobeHub 需要的是 bot 的，会在第二步通过 LINE API 取出。
...
## 第二步：签发 Channel Access Token 并获取 Destination User ID
...
<Steps>
  ...
  ### 通过 API 获取 Destination User ID

  LINE Developers Console 当前界面**不再展示** bot 的 destination user ID，必须用刚拿到的 token 调用 LINE API 取出：
  ...
  其中 **`userId`** 字段（以 `U` 开头共 33 位）就是 LobeHub 需要的 **Destination User ID**，也是每条入站 Webhook payload 中的 `destination`。复制下来供第四步使用。
</Steps>
...
<Steps>
  ...
  | 字段                       | 值                                       |
  | ------------------------ | --------------------------------------- |
  | **Destination User ID**  | 第二步通过 `/v2/bot/info` 拿到的 `userId`（`U…`） |
...
| 字段                       | 是否必需 | 描述                                                                                                           |
| ------------------------ | ---- | ------------------------------------------------------------------------------------------------------------ |
| **Destination User ID**  | 是    | 机器人的用户 ID（`U` + 32 位十六进制）。通过 `GET /v2/bot/info` 取得 ——LINE Developers Console 界面不展示这个值。同时作为机器人标识与 Webhook 路径。 |
...
- **`Channel access token belongs to bot Uxxx, not Uyyy`。** Destination User ID 与 Token 不匹配。错误消息里的 `Uxxx` 就是 Token 实际归属的 bot userId，直接复制它填进 Destination User ID 字段即可（也可以用 `curl -H "Authorization: Bearer <token>" https://api.line.me/v2/bot/info` 自行确认）。
