...
Slack DMs are scoped to the workspace, so by default LobeHub **opens** DM handling for Slack — anyone in the workspace can DM the bot. Adjust **DM Policy** in **Advanced Settings** to tighten this:

- **Open (default)** — Any workspace member can DM the bot.
- **Allowlist** — Only the Slack member IDs you list in **Allowed User IDs** can DM the bot. Open a user's profile → click `⋮` → **Copy member ID** (the ID starts with `U`).
- **Disabled** — The bot ignores all DMs and only replies to channel `@mentions`.

DM Policy only governs DMs. Channel `@mentions` are unaffected. See the [Channels overview](/docs/usage/channels/overview#direct-message-policy) for cross-platform details.
...
| **Allowed User IDs** | No               | Comma- or whitespace-separated Slack member IDs. Only used when DM Policy is Allowlist |
