...
LobeHub 默认对 Discord **开放** 私信 — 任何与机器人共享服务器的用户都可以私信。如需收紧，可以在渠道表单的 **高级设置** 调整 **私信策略 (DM Policy)**：

- **开放 (Open)（默认）** — 任何与机器人共享服务器的用户都可以私信。
- **白名单 (Allowlist)** — 只有列在 **允许的用户 ID** 里的 Discord 用户可以私信。在 **设置 → 高级** 启用开发者模式后，右键用户 → **复制用户 ID** 即可获取。
- **禁用 (Disabled)** — 机器人忽略所有私信，只在共享服务器里被 `@提及` 时回复。

> Discord 机器人可被任意共享服务器的用户私信，如果你的机器人是私有用途，建议切到 **白名单** 或 **禁用**。
