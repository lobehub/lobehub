...
Every supported platform defaults to **Open** so the bot stays reachable out of the box. Tighten this per-channel via **Allowlist** or **Disabled** when you want a private bot. WeChat does not expose this setting because the WeChat integration is DM-only by design.
