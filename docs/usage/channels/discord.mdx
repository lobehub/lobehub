...
By default LobeHub **opens** DM handling for Discord — anyone who shares a server with the bot can DM it. Adjust **DM Policy** in **Advanced Settings** if you want to restrict DM access:

- **Open (default)** — Anyone who shares a server with the bot can DM it.
- **Allowlist** — Only the Discord user IDs you list in **Allowed User IDs** can DM the bot. Right-click a user → **Copy User ID** (Developer Mode must be enabled in **Settings → Advanced**) to grab an ID.
- **Disabled** — The bot ignores all DMs. It only responds to `@mentions` in shared servers.

> Discord bots can be reached by anyone in any shared server, so consider switching to **Allowlist** or **Disabled** if your bot is meant to be private.

DM Policy only governs DMs. Server-channel `@mentions` are unaffected. See the [Channels overview](/docs/usage/channels/overview#direct-message-policy) for cross-platform details.
...
| **Allowed User IDs** | No       | Comma- or whitespace-separated Discord user IDs. Only used when DM Policy is Allowlist |
...
- **Bot not responding to DMs:** Open **Advanced Settings** and confirm **DM Policy** is not set to `Disabled`. If it is `Allowlist`, make sure the sender's Discord user ID is in **Allowed User IDs**.
