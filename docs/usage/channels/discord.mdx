...
Discord bots can be DM'd by anyone who shares a server with them, so by default LobeHub **disables** DM handling for Discord. To allow DMs, open **Advanced Settings** in the channel form and change **DM Policy**:

- **Disabled (default)** — The bot ignores all DMs. It only responds to `@mentions` in shared servers.
- **Open** — Anyone who shares a server with the bot can DM it.
...
| **DM Policy**        | No       | `disabled` (default), `open`, or `allowlist` — who is allowed to DM the bot            |
