...
- A [LINE Business ID](https://account.line.biz/login) (any LINE account can sign in)
- A **Provider** in the [LINE Developers Console](https://developers.line.biz/console/) — create one if this is your first bot
...
<Steps>
  ...
  Sign in to [developers.line.biz/console](https://developers.line.biz/console/) and either pick an existing Provider or click **Create a new provider** and give it a name (e.g. "LobeHub").

  ### Create a Messaging API channel

  Inside the Provider, click **Create a new channel** → **Messaging API**. Fill in the channel name, description, category, and region. Submit.

  ### Note the channel identifiers

  After the channel is created, open the **Basic settings** tab. You will use:

  - **Channel secret** — copy this for Step 4 (LobeHub uses it to verify webhook signatures).

  Then open the **Messaging API** tab. Note:

  - **Bot basic ID** — the `@xxxx` short ID users will search for.
  - **Bot info** section displays the **destination user ID** (`U` + 32 hex chars). This is what LobeHub needs as the **Destination User ID**.
</Steps>
...
<Steps>
  ...
  | Field                    | Value                                                |
  | ------------------------ | ---------------------------------------------------- |
  | **Destination User ID**  | The bot's `U…` user ID from Messaging API → Bot info |
...
| ------------------------ | -------- | ------------------------------------------------------------------------------------------------------------------------------------ |
| **Destination User ID**  | Yes      | The bot's user ID (`U` + 32 hex chars) from **Messaging API → Bot info**. Used as the bot identity and the webhook URL path segment. |
...
- **`Channel access token belongs to bot Uxxx, not Uyyy`.** The Destination User ID does not match the token. Open **Messaging API → Bot info** and copy the exact `U…` value into the Destination User ID field.
