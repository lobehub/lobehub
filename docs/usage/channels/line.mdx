...
<Steps>
  ...
  > **Note:** the **"Your user ID"** field on the same tab is **your own** LINE user ID, **not** the bot's destination user ID. Both have the identical `U` + 32 hex format, but LobeHub needs the bot's, which we'll fetch via the LINE API in Step 2.
...
## Step 2: Issue a Channel Access Token and Fetch the Destination User ID
...
<Steps>
  ...
  ### Fetch the bot's Destination User ID via API

  The LINE Developers Console **does not display** the bot's destination user ID anywhere. Use the token you just issued to call the LINE API:
  ...
  The **`userId`** field (33 chars, starts with `U`) is the **Destination User ID** LobeHub needs, and matches the `destination` field on every inbound webhook payload. Copy it for Step 4.
</Steps>
...
<Steps>
  ...
  | Field                    | Value                                             |
  | ------------------------ | ------------------------------------------------- |
  | **Destination User ID**  | The `userId` returned by `/v2/bot/info` in Step 2 |
...
| ------------------------ | -------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Destination User ID**  | Yes      | The bot's user ID (`U` + 32 hex chars). Fetched via `GET /v2/bot/info` — the LINE Developers Console UI does not display this value. Used as the bot identity and webhook path segment. |
...
- **`Channel access token belongs to bot Uxxx, not Uyyy`.** The Destination User ID does not match the token. The `Uxxx` shown in the error is the userId the token actually belongs to — copy it directly into the Destination User ID field. You can also re-confirm with `curl -H "Authorization: Bearer <token>" https://api.line.me/v2/bot/info`.
