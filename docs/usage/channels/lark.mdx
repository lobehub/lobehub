...
Lark apps are scoped to a tenant, so LobeHub **opens** DM handling for Lark by default — anyone in the tenant who can see the bot can DM it. Adjust **DM Policy** in **Advanced Settings**:

- **Open (default)** — Any tenant member can DM the bot.
- **Allowlist** — Only the Lark user IDs you list in **Allowed User IDs** can DM the bot. Use the `open_id` from the event payload, or the **User ID** displayed in the Lark Developer Portal.
...
| **Allowed User IDs**       | No       | Comma- or whitespace-separated Lark `open_id` values. Only used when DM Policy is Allowlist |
