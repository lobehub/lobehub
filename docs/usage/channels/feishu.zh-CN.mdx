...
飞书应用绑定在租户内，因此 LobeHub 默认对飞书 **开放** 私信 — 租户内任何能看到机器人的成员都可以私信。如需收紧，可以在 **高级设置** 调整 **私信策略 (DM Policy)**：

- **开放 (Open)（默认）** — 租户内任何成员都可以私信机器人。
- **白名单 (Allowlist)** — 只有列在 **允许的用户 ID** 里的飞书用户可以私信。使用事件 payload 里的 `open_id`，或飞书开发者后台显示的 **User ID**。
...
| **允许的用户 ID**           | 否    | 逗号或空格分隔的飞书 `open_id`。仅在私信策略为白名单时生效               |
