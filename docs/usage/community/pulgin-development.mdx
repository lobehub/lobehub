...
description: Learn how to add and use custom plugins in LobeHub, including creating a plugin project, adding local plugins in role settings, testing plugin functionality, and understanding the development and deployment process.
tags:
  - LobeChat
  - LobeHub
  - Plugin Development
...
After completing these steps, you now understand the basic process of adding and using a custom plugin in LobeChat.
</Steps>
...
4. `gateway`: Specifies the API gateway. LobeChat’s default is a cloud service, but for local plugins, this should point to your local server.
...
LobeChat’s default plugin gateway is `/api/plugins`, which sends requests to the `api.url` in the manifest.
...
[`@lobehub/chat-plugins-gateway`](https://github.com/lobehub/chat-plugins-gateway) provides the gateway implementation used in LobeChat. Use it to route requests to your local plugin service.
...
To simplify communication, use `fetchPluginMessage` from [`@lobehub/chat-plugin-sdk`](https://github.com/lobehub/chat-plugin-sdk). It fetches the current plugin message from LobeChat. See [fetchPluginMessage][fetch-plugin-message-url] for details.
