...
description: >-
  Learn how to add and use custom plugins in LobeChat, including creating a
...
2. **Plugin Manifest**: Describes the plugin’s functionality, including backend services, frontend UI, version, and more. For a detailed explanation, see the [manifest documentation][manifest-docs-url].
...
This section walks you through how to add and use a custom plugin in LobeChat.
...
Next, open LobeChat, create a new assistant, and go to its session settings:
...
Click <kbd>Back</kbd> to return to the chat window and send a message like: “What should I wear today?” The assistant will ask for your gender and current mood.
...
After you respond, the assistant will call the plugin, fetch clothing recommendations based on your mood and gender, and display them. It will also provide a summary.
...
With that, you’ve completed the basic process of adding and using a custom plugin in LobeChat.
</Steps>
      ...
      "parameters": {
        "properties": {
          ...
          "gender": {
            "type": "string",
            "enum": ["man", "woman"],
            "description": "User's gender, must be asked before use"
          }
        },
...
3. `ui`: Defines the plugin’s frontend interface. LobeChat loads this via an iframe. You can specify height and width.
4. `gateway`: Specifies the gateway for API requests. Since remote servers can’t access localhost, this field allows LobeChat to route requests to your local service. Not required for published plugins.
5. `version`: Plugin version (currently unused).

For full manifest documentation, see: [manifest][manifest-docs-url].
...
We also welcome contributions of templates in other frameworks or languages.
...
LobeChat’s default plugin gateway is the cloud endpoint `/api/plugins`. For local plugins, you must define a custom gateway in the manifest.
...
The [`@lobehub/chat-plugins-gateway`](https://github.com/lobehub/chat-plugins-gateway) package provides the gateway implementation used in LobeChat. Use it to route requests to your local plugin service.

### Plugin UI

The UI is optional. For example, the official [🧩 / 🕸 Web Content Extractor](https://github.com/lobehub/chat-plugin-web-crawler) plugin has no UI.
...
If you want to display rich content or interactive elements, you can build a custom UI. Below is the UI for the [Search Engine Plugin](https://github.com/lobehub/chat-plugin-search-engine).
...
LobeChat loads plugin UIs via `iframe` and uses `postMessage` for communication. You can use any frontend framework or language.
...
To simplify communication, we provide helper methods in [`@lobehub/chat-plugin-sdk`](https://github.com/lobehub/chat-plugin-sdk). Use `fetchPluginMessage` to retrieve the current message data.
...
To share your plugin with others, submit it to the [plugin marketplace](https://github.com/lobehub/lobe-chat-plugins).
...
- **📘 Plugin SDK Docs**: <https://chat-plugin-sdk.lobehub.com>
