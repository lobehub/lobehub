...
<Callout type={'info'}>
  Because it's built in, Web Search works the same way across every Agent — no API key, plugin
  install, or MCP server setup required to get started.
</Callout>
...
<Callout type={'info'}>
  Web search works out of the box via the built-in Web Search Skill — no plugin or MCP setup
  required. If you connect an optional MCP search server instead, that provider may require its
