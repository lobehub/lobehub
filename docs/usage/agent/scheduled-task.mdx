...
> **Self-hosting note** To make scheduled tasks run automatically in self-hosted deployments, you also need a scheduler trigger (for example: Vercel Cron, QStash schedule, or a Docker sidecar that periodically calls the dispatcher endpoint).
