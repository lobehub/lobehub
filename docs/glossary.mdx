...
A reasoning style where the model works through a problem step by step before answering. LobeHub shows this thinking in an expandable section as it streams.
...
The marketplace area for browsing and installing community-published agents, skills, MCP servers, models, and providers. Also called the Marketplace.

### DM (Direct Message)

A private message thread with a single member agent inside a group, visible only to you.
...
A continuous voice conversation mode that pairs spoken input (STT) with spoken responses (TTS) so you can talk with an agent without typing or reading.
