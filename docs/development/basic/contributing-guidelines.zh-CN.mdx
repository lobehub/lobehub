...
- **AI** — 人写的改动删掉这一节。Agent 写的改动，如果作者是外部 collaborator 或其他外部贡献者，模板里的注释指令会要求这个 agent 填写 harness、模型、thinking level，以及这次会话里的全部 user prompt（按顺序、原文照录）。lobehub organization 的正式成员删掉这一节。
