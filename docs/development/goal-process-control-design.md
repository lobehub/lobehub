...
**Date:** 2026-08-26
**Status:** aligning — round 6 (per-step polish; prototype split into components; rounds 1–5 refined by the user)
...
| Human gates missing from the map                      | Budget top-up and final acceptance are not decision nodes today (❌) — modeled as NEW decision nodes so the map keeps every human/agent decision, with 你 / AI badges.                                                     | implementation |
