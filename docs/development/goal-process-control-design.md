...
**Date:** 2026-08-26
**Status:** aligning — round 13 (graph rebuilt on react-flow with product-grade card nodes)
...
Round-12 corrections:

| Correction (user)                                   | What changed                                                                                                                                                                                                                                                                                                                                                                                                                                             |
| --------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| A row that needs a decision must open, not truncate | Every needs-you row (gate /delivered/budget/lost contact) renders an expanded block: 为什么停下来・每个选择意味着什么 (option → consequence, recommended marked)・之前的尝试・补充说明. Nothing is squeezed onto the title line.                                                                                                                                                                                                                         |
| The activity feed is too noisy                      | It is now a **per-node work record**, not a raw event log: one row per task — actor · what state it reached (`2 次尝试都没通过验证，等你决定，另有 1 次失联回收`) · time — with attempts, findings and human touches folded underneath. Verifier verdicts, evidence submission and lease recovery happen _inside_ a task, so they live in that task's fold. Only goal-level events (created / paused / budget / comment / achieved) keep their own rows. |

Round-13 — the graph as product-grade cards:

Each node is a card: a tinted square with the kind's icon, the title plus a one-line subtitle, and a
state chip on the top edge (进行中 / 完成 / 等你决定 / 失联 / AI 决定). A **Work** card adds a metric
strip: the executing agent's avatar + name, attempt count, whether an independent verifier judges its
delivery, how many artifacts its attempts registered, and the running clock. `hasVerifier` and
`artifacts` are new view fields — in production they come from the Work-scoped acceptance and the
registered work versions (⚠️ both already modeled, never surfaced). Edges carry their relation label
(依赖 / 产出 / 支持 / 导向) so the map reads without an edge legend.
