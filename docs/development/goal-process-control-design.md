...
**Date:** 2026-08-26
**Status:** aligning — round 5 (the prototype now replays the whole flow step by step; rounds 1–4 refined by the user)
...
**Prototype:** `docs/development/goal-process-control-prototype.html` — a 20-step replay of one real Goal (上一步 / 下一步 or ← →), each step showing the page state right after that moment; nodes/cards changed in the step glow. Steps map onto §0 as: 1 create・2–3 first advance/execute・4 pass → findings・5–6 next Work + agent-authority decision・7–8 training・9–10 lost contact → reclaim・11 fail → automatic retry・12–13 budget boundary → top-up・14 gate・15 decide・16 pass・17 remaining Works・18 goal acceptance・19 delivered → confirm・20 achieved. 加载中 / 加载失败 are reachable from the bar.
