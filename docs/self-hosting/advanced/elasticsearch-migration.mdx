...
- Upgrading to a new schema version reuses this same resumable backfill into new, version-suffixed physical indexes, plus a `--switch-aliases` flag for the atomic cutover; see [Schema version upgrades](#schema-version-upgrades).
...
example `v1` to `v2`), using the same resumable backfill mechanics as the initial migration plus a
`--switch-aliases` flag that atomically cuts every stable alias over to the new physical indexes.
`--switch-aliases` is only accepted together with `--apply --yes`. The default reindex behavior is
unchanged: without `--switch-aliases`, the command still fails closed if a stable alias already
points to a different physical index. The command also refuses `--switch-aliases` while the Outbox
has in-flight claims; pause the incremental sync consumer first.
...
Follow this procedure for an online, no-downtime upgrade:

1. Pause the incremental sync consumer (set `FTS_SEARCH_SYNC_ENABLED=false` or stop the
   `fts-search:sync` process). Product search keeps serving from the current stable aliases, and the
   Outbox keeps accumulating changes; nothing is lost.

2. Start a fresh backfill into the new schema version's physical indexes:

   ```bash
   bun run fts-search:reindex -- --apply --fresh-run --yes
   ```

   A schema-version bump always needs a new checkpoint (`--fresh-run`), because it targets new
   version-suffixed physical indexes (for example `-v2`) written with `external_gte` versioning from
   a base revision reserved through the write fence. This run is resumable exactly like an initial
   migration.

3. After the backfill reaches `ready_for_incremental_sync`, cut over atomically:

   ```bash
   bun run fts-search:reindex -- --apply --switch-aliases --yes
   ```

   This repoints all stable aliases from the old physical indexes to the new ones in a single atomic
   `_aliases` request. Passing `--switch-aliases` from the start in step 2 performs the same cutover
   automatically as soon as the backfill completes, instead of requiring this separate invocation.

4. Resume the incremental sync consumer. Pending Outbox rows replay into the new schema version's
   indexes; rows older than what the backfill already captured are rejected by Elasticsearch as
   version conflicts (409), which the sync consumer treats as success.

5. After verifying search, delete the old physical indexes manually, for example:
...
To roll back before step 4, repoint the stable aliases back to the old physical indexes manually via
`_aliases`, then resume the sync consumer.
