...
- Upgrading to a new schema version normally copies documents directly inside Elasticsearch with `--copy-from-schema-version`, preserving document versions and reading nothing from PostgreSQL; a `--switch-aliases` flag performs the atomic cutover. When the source schema version no longer keeps every field in `_source`, the upgrade instead falls back to this same resumable backfill into new, version-suffixed physical indexes. See [Schema version upgrades](#schema-version-upgrades).
...
The reindex tool supports an online, no-downtime upgrade of the search index schema version (for
example `v1` to `v2`) by copying documents directly inside Elasticsearch, without re-scanning
PostgreSQL. Pass `--copy-from-schema-version=<N>`, where `N` must be older than the schema version
currently built into the tool. The flag is mutating and requires `--yes`, and it is mutually
exclusive with `--apply`, `--status`, and `--skip-failure`. For each entity, it reindexes the
`<namespace>-<entity>-v<N>` physical index into `<namespace>-<entity>-v<current>` using
Elasticsearch's own `_reindex` API with `conflicts: proceed` and `version_type: external`, so
document versions carry over and no PostgreSQL row is read. It creates the target physical indexes
with the current mappings when they do not already exist, waits for each entity's reindex task,
refreshes the target, and prints per-entity source/target document counts plus the updated and
version-conflict counts.

Because document versions are preserved, running `--copy-from-schema-version` again is safe: it
only rewrites documents whose version changed since the previous run, and reports every other
document as a version conflict rather than an error.

`--switch-aliases` can be combined with `--copy-from-schema-version`. Immediately before switching,
the command checks the Outbox for in-flight claims and refuses to proceed if any exist, so the
incremental sync consumer must be paused first. The switch moves every stable alias to the new
physical indexes in one atomic `_aliases` request.

<Callout type="warning">
  The in-place copy only works when the SOURCE schema version still stores every mapped field in
  `_source`. Schema version `v1` does; `v2` and later versions do not, because they exclude long
  text fields from `_source`. So `v1` to `v2` can use `--copy-from-schema-version`, but any later
  upgrade that changes how an already-excluded field is indexed (a different analyzer or field
  type) cannot reconstruct that field from `_source` and must fall back to a full PostgreSQL
  backfill: `--apply --fresh-run --yes`, then `--apply --switch-aliases --yes` once it reaches
  `ready_for_incremental_sync`, with the incremental sync consumer paused for the whole backfill.
</Callout>
...
Follow this procedure for the `v1` to `v2` upgrade:

1. Keep the incremental sync consumer running. Start the copy:

   ```bash
   bun run fts-search:reindex -- --copy-from-schema-version=1 --yes
   ```
...
2. Pause the incremental sync consumer (set `FTS_SEARCH_SYNC_ENABLED=false` or stop the
   `fts-search:sync` process). The Outbox keeps accumulating changes; nothing is lost.

3. Run the copy again with `--switch-aliases`:

   ```bash
   bun run fts-search:reindex -- --copy-from-schema-version=1 --switch-aliases --yes
   ```

   This second pass only copies documents that changed since step 1, so it finishes quickly, then
...
4. Resume the incremental sync consumer. Pending Outbox rows replay into the `v2` indexes; anything
   already newer in `v2` is rejected by Elasticsearch as a version conflict (409), which the sync
   consumer treats as success.

5. After verifying search, delete the old `v1` physical indexes manually, for example:
...
To roll back before step 4, repoint the stable aliases back to the `v1` physical indexes manually
