...
| ------------- | ------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------- |
| `anysearch`   | [AnySearch](https://www.anysearch.com/) Extract API for cleaned page content. Anonymous access is supported.        | `ANYSEARCH_API_KEY` (optional)                                              |
...
| Value        | Description                                                                                  | Environment Variable                                                        |
| ------------ | -------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------- |
| `anysearch`  | [AnySearch](https://www.anysearch.com/) real-time web search. Anonymous access is supported. | `ANYSEARCH_API_KEY` (optional)                                              |
...
AnySearch can provide both built-in web search and page extraction. An API key is optional; without one, requests use AnySearch's anonymous quota.

```env
SEARCH_PROVIDERS="anysearch"
CRAWLER_IMPLS="anysearch"
# Optional
ANYSEARCH_API_KEY=as_sk_your_key
```

The built-in Web Search path uses AnySearch general search. LobeHub lets the provider choose its own backing search source, so explicit engine restrictions such as Google or Bing are not forwarded to AnySearch.

For AnySearch vertical-domain search and parallel batch search, add the official remote MCP server in **Settings → Connectors** using `https://api.anysearch.com/mcp`. The MCP server exposes domain discovery, vertical search, batch search, and Extract. It also supports anonymous access; for authenticated usage, store the `Authorization: Bearer <API_KEY>` header in the Connector authentication settings rather than in shared configuration.
