...
4. **通知用户**
   ...
   告知用户按以下步骤重置密码（聊天记录和设置将被保留）：

   1. 访问登录页（如 `https://your-domain.com/signin`）
   2. 输入之前使用的邮箱，回车
   3. 页面会展示密码输入框，点击下方的「忘记密码」
   4. 点击收到的重置密码邮件中的链接进行密码重置
   5. 使用重置后的邮箱密码登录
   6. （可选）在个人资料页 → 已关联账号 部分可手动关联 Google、GitHub 等账号

   ![LobeChat 登录页面 - 忘记密码入口](https://hub-apac-1.lobeobjects.space/docs/8d0563701fcd17ad7252a72b1746dd42.png)

<Callout type={'tip'}>
  这种方法快速且配置简单。用户只需重置密码即可继续使用 LobeChat，所有数据完整保留。
</Callout>
...
<Callout type={'warning'}>
  **强烈建议**：在正式迁移前，先使用 test 模式验证脚本能正常运行：

  - 使用 [Neon Fork 分支](https://neon.tech/docs/manage/branches#create-a-branch) 创建测试数据库
  - 使用 Clerk Development 环境的 API 密钥
  - 确认测试迁移成功后再切换到 prod 模式
</Callout>

### 前置条件

- Node.js 18+
- 访问 Clerk 控制台（用于 CSV 导出）
- Clerk API 密钥（用于 API 导出）
- 数据库连接字符串
- 数据库 schema 已更新至最新版本
- 已阅读 [身份验证服务配置](/zh/docs/self-hosting/advanced/auth) 文档，了解 Better Auth 环境变量

<Callout type={'info'}>
  如果你长期停留在旧版本（如 1.x），数据库 schema 可能不是最新的。在运行迁移脚本前，请先更新数据库 schema：

  ```bash
  DATABASE_URL=your-database-url pnpm db:migrate
  ```

  详见 [数据库迁移文档](/zh/docs/self-hosting/server-database/docker-compose#upgrade)。
</Callout>
