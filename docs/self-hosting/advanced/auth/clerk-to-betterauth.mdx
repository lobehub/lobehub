...
4. **Notify Users**
   ...
   Inform users to follow these steps to reset their password (chat history and settings will be preserved):

   1. Visit the login page (e.g., `https://your-domain.com/signin`)
   2. Enter their previous email address and press Enter
   3. The page will show a password input field, click "Forgot Password" below it
   4. Click the link in the password reset email to reset their password
   5. Log in using the new email and password
   6. (Optional) Go to Profile → Linked Accounts to manually link Google, GitHub, or other accounts

   ![LobeChat Login Page - Forgot Password](https://hub-apac-1.lobeobjects.space/docs/8d0563701fcd17ad7252a72b1746dd42.png)

<Callout type={'tip'}>
  This method is quick and requires minimal setup. Users simply reset their passwords and continue using LobeChat with all their data intact.
</Callout>
...
<Callout type={'warning'}>
  **Strongly recommended**: Before production migration, verify the scripts work correctly in test mode:
  ...
  - Only switch to prod mode after confirming test migration succeeds
</Callout>
...
- Familiar with [Authentication Service Configuration](/docs/self-hosting/advanced/auth) for Better Auth environment variables

<Callout type={'info'}>
  If you've been on an older version (e.g., 1.x) for a while, your database schema may be outdated. Before running the migration scripts, update your database schema first:

  ```bash
  DATABASE_URL=your-database-url pnpm db:migrate
  ```

  See [Database Migration Documentation](/docs/self-hosting/server-database/docker-compose#upgrade) for details.
</Callout>
