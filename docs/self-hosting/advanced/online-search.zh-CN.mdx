...
| 值             | 说明                                                                        | 环境变量                                                                        |
| ------------- | ------------------------------------------------------------------------- | --------------------------------------------------------------------------- |
| `anysearch`   | 使用 [AnySearch](https://www.anysearch.com/) Extract API 提取清理后的网页内容，支持匿名访问。 | `ANYSEARCH_API_KEY`（可选）                                                     |
...
| 值            | 说明                                                                                    | 环境变量                                                                        |
| ------------ | ------------------------------------------------------------------------------------- | --------------------------------------------------------------------------- |
| `anysearch`  | 使用 [AnySearch](https://www.anysearch.com/) 提供实时网络搜索，支持匿名访问。                           | `ANYSEARCH_API_KEY`（可选）                                                     |
...
AnySearch 可以同时为内置网络搜索和网页内容提取提供服务。API Key 为可选配置；未配置时会使用 AnySearch 的匿名额度。
...
内置 Web Search 使用 AnySearch 的通用搜索。LobeHub 会让 AnySearch 自行选择底层搜索来源，因此 Google、Bing 等显式搜索引擎限制不会转发给 AnySearch。

如果需要 AnySearch 的垂直领域搜索和并行批量搜索，可在**设置 → 连接器**中添加官方远程 MCP 服务器 `https://api.anysearch.com/mcp`。该 MCP 提供领域发现、垂直搜索、批量搜索和 Extract，并支持匿名访问；如需认证，请在连接器认证设置中保存 `Authorization: Bearer <API_KEY>`，不要写入共享配置。
