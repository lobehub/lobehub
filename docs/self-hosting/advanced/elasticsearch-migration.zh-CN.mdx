...
`--apply` 会先校验 Memory 扇出所需的 GIN 索引，再逐表安装增量采集触发器；每次获取表锁最多等待 3 秒。首次执行应安排在低流量时段。如果某张表未能及时获得短锁，命令会在开启采集前停止，使用同一个状态目录重新执行即可。全部采集设施就绪后，命令才会在首批回填前开启 PostgreSQL 增量队列采集。必须显式传入 `--yes`，用于确认这个持久化数据库变更；命令不会弹出交互确认。
