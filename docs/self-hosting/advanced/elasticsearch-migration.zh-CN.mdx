...
- 升级结构版本通常会直接在 Elasticsearch 内部复制文档（`--copy-from-schema-version`），保留文档版本号，且完全不读取 PostgreSQL；`--switch-aliases` 参数负责原子切换。如果源结构版本已经不再把全部字段保存在 `_source` 中，升级会改为回退到同一套可续跑的回填机制，写入带新版本号后缀的物理索引。详见[结构版本升级](#结构版本升级)。
...
回填工具支持在线、不停机地升级搜索索引结构版本（例如从 `v1` 升级到 `v2`）：直接在 Elasticsearch 内部复制文档，不需要重新扫描 PostgreSQL。传入 `--copy-from-schema-version=<N>`，其中 `N` 必须早于工具当前内置的结构版本。该参数会写入数据，必须搭配 `--yes`，并且与 `--apply`、`--status`、`--skip-failure` 互斥。对每个数据类型，它会使用 Elasticsearch 自身的 `_reindex` API（`conflicts: proceed`、`version_type: external`），把 `<命名空间>-<数据类型>-v<N>` 物理索引复制到 `<命名空间>-<数据类型>-v<当前版本>`，因此文档版本号会被保留，也不会读取任何 PostgreSQL 数据。如果目标物理索引尚不存在，命令会先按当前 mapping 创建；然后等待每个数据类型的 reindex 任务完成、刷新目标索引，并打印每个数据类型的源 / 目标文档数，以及更新数量和版本冲突数量。

由于文档版本号被保留，重复执行 `--copy-from-schema-version` 是安全的：再次执行只会重写自上一次执行后版本发生变化的文档，其余文档都会被报告为版本冲突，而不是错误。

`--switch-aliases` 可以和 `--copy-from-schema-version` 一起使用。切换别名前，命令会检查增量队列中是否存在正在处理的任务，如果存在就拒绝执行，因此必须先暂停增量消费器。切换会在一次原子 `_aliases` 请求中移动全部稳定别名。

<Callout type="warning">
  这种就地复制只在源结构版本仍然把全部映射字段保存在 `_source` 中时才有效。`v1` 满足这个条件，但 `v2` 及之后的版本不满足，因为它们会把长文本字段从 `_source` 中排除。因此 `v1` 升级到 `v2` 可以使用 `--copy-from-schema-version`，但之后任何改变了某个已被排除字段索引方式（分析器或字段类型变化）的升级，都无法从 `_source` 重建该字段，必须回退到完整的 PostgreSQL 全量回填：先执行 `--apply --fresh-run --yes`，等状态变为 `ready_for_incremental_sync` 后再执行 `--apply --switch-aliases --yes`，并在整个回填期间保持增量消费器暂停。
</Callout>

例如，结构版本 `2` 会把长文本字段从 `_source` 中排除，但这些字段仍会被索引、可被搜索。应用只从 Elasticsearch 读取候选文档 ID，再从 PostgreSQL 补全完整内容，因此不会影响用户可见的结果，同时单个索引的存储占用会大幅下降。
...
2. 暂停增量消费器（设置 `FTS_SEARCH_SYNC_ENABLED=false`，或停止 `fts-search:sync` 进程）。增量队列会持续积累变更，不会丢失数据。

3. 带上 `--switch-aliases` 再执行一次复制：

   ```bash
   bun run fts-search:reindex -- --copy-from-schema-version=1 --switch-aliases --yes
   ```

   这一次只会复制第 1 步之后发生变化的文档，因此耗时很短；完成后会立即原子切换全部稳定别名到 `v2` 物理索引。这段短暂窗口是搜索数据滞后于源数据的唯一时间段。

4. 恢复增量消费器。增量队列中待处理的记录会重放写入 `v2` 索引；如果 `v2` 中已经有更新的版本，Elasticsearch 会将其判定为版本冲突（409）并拒绝，消费器会把这种情况视为成功。

5. 确认搜索正常后，手动删除旧的 `v1` 物理索引，例如：
...
如果需要在第 4 步之前回滚，可以手动通过 `_aliases` 把稳定别名指回 `v1` 物理索引，再恢复增量消费器。
