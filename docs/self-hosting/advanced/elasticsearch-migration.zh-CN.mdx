...
<Callout type="warning">
  全量回填命令不会把产品搜索请求切到 Elasticsearch。回填和验收期间，应用必须继续使用 PostgreSQL 搜索。常规数据库迁移只创建 PostgreSQL 增量队列的基础结构，不会安装搜索变更采集触发器。只有显式执行 `bun run db:install-search-capture` 或启动 `bun run search:reindex -- --apply` 后才会开始采集，即使增量消费器仍处于暂停状态也是如此。
</Callout>
...
2. 执行同一 LobeHub 版本自带的数据库迁移：

   ```bash
   bun run db:migrate
   ```

   这一步只创建增量队列的序列、表和普通索引，不会创建 Memory 扇出所需的 GIN 索引，也不会创建 PostgreSQL 变更采集函数和触发器。只使用 PostgreSQL 搜索的部署可以停在这里，不会承担搜索变更采集的写入开销。

3. 在启用 Elasticsearch 采集前备份 PostgreSQL。

4. 如果准备在仍有大量写入的现有数据库上启用 Elasticsearch，请先在数据库 SQL 编辑器中、事务之外执行以下语句。它会在不阻塞写入的情况下创建 Memory 扇出索引，后续安装器可以直接复用。全新或低流量安装可以跳过这一步；安装器会以幂等方式创建同一个索引：
   ...
   `IF NOT EXISTS` 不会校验同名索引的定义。如果并发创建留下无效索引，或 `--apply` 提示索引的表、列、GIN 操作符类或条件不正确，请在事务之外执行 `DROP INDEX CONCURRENTLY "user_memories_contexts_user_memory_ids_gin_idx"`，再重新执行上面的创建语句。
...
5. 在首次回填前安装可选的 PostgreSQL 变更采集：
   ...
   这一步会安装 GIN 索引、采集函数和触发器。安装器是幂等的，部署中断后可以安全重试；成功后源数据变更会立即写入增量队列。`bun run search:reindex -- --apply` 也会自动执行这一步；如果希望在准备 Elasticsearch 目标期间就开始采集，可以先显式执行安装命令。若不打算启用 Elasticsearch，不要执行这一步。
...
`--apply` 会先创建或校验所有 Elasticsearch 物理索引及其 ICU 分析设置，并以幂等方式安装可选的 PostgreSQL 变更采集设施（如果之前已经安装则复用），然后校验 Memory 扇出所需的 GIN 索引，再读取源数据。只读的 `--status` 不会安装或修改这些设施。读取源数据前，`--apply` 会短暂阻挡所有触发器源表的写入，等待持有较旧 Outbox 版本的事务结束；如果三秒内拿不到锁，应先让长事务结束，再重新运行命令。首次回填期间可以暂停增量消费器；PostgreSQL 仍会把变更追加到增量队列，完整快照准备好后再由消费器追平。必须显式传入 `--yes`，用于确认这个会写入数据的操作；命令不会弹出交互确认。
...
继续下一阶段前，运行 `--status`，确认 14 类数据全部为 `completed`、所有 `failedCount` 都是 `0`，且增量队列的 `dead` 为 `0`。增量消费器暂停时，队列中有待处理记录是预期现象；这不代表源数据变更采集已暂停。状态会列出未解决记录的文档标识、是否可重试和尝试次数，不会保存或输出可能包含原文的 Elasticsearch 错误原因。
