...
2. 对于现有正式数据库，请先在数据库 SQL 编辑器中、事务之外执行以下语句，再执行当前版本的数据库迁移。全新安装可以在数据库迁移完成后执行，但必须在首次运行回填 `--apply` 前完成。这样可以在不阻塞写入的情况下创建 Memory 扇出索引：
...
`--apply` 会先创建或校验所有 Elasticsearch 物理索引及其 ICU 分析设置，再校验 Memory 扇出所需的 GIN 索引，并逐表安装增量采集触发器；每次获取表锁最多等待 3 秒。首次执行应安排在低流量时段。全部采集设施就绪后，启用操作会获取所有采集表的共享锁，并在同一事务中开启 PostgreSQL 增量队列采集，随后才开始首批回填。如果仍有启用前的写事务，保护锁会立即失败，避免迁移阻塞生产写入。如果 Elasticsearch 分析能力、某张表的触发器检查或启用保护锁失败，命令会保持采集关闭，使用同一个状态目录重新执行即可。必须显式传入 `--yes`，用于确认这个持久化数据库变更；命令不会弹出交互确认。
...
- 进程崩溃或 Elasticsearch 临时失败后，使用同一个状态目录重新执行 `--apply --yes`，但不要再传 `--fresh-run`。已完成的数据类型会被跳过，未完成的数据类型从本地游标后继续。只有首次持久化批次之后采集始终保持开启，这种续跑才安全。
...
对于已经产生持久化进度的 checkpoint，关闭采集后不能再续跑。采集中断期间的变更不会进入队列，因此工具会拒绝继续使用旧 checkpoint。若要在采集中断后重新开始，必须使用新的 checkpoint 和空 Elasticsearch 目标执行完整回填。
