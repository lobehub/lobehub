...
全新迁移始终以当前结构版本 `v2` 为目标（版本记录见下文的[结构版本升级](#结构版本升级)）。工具会创建 `lobehub-messages-v2` 这类实体索引；14 类索引全部完成且数量与断点记录一致后，再创建 `lobehub-messages` 这类稳定别名。别名只是 Elasticsearch 内部长期不变的名字，创建别名不会改变用户请求当前使用的搜索后端。物理索引名称始终包含结构版本号，稳定别名则永远不包含。
...
- `backfill`：回退到与初始迁移相同的可续跑 PostgreSQL 回填机制，需要设置 `ES_REINDEX_STATE_DIR`。第一次执行会创建全新 checkpoint，之后的执行会继续该 checkpoint。不带 `--switch-aliases` 时，会停在 `awaiting_alias_switch` 状态，旧版本的别名继续对外提供服务。这种策略下，必须在整个回填期间保持增量消费器暂停 —— 期间发生的变更会保留在增量队列中，并在切换后重放 —— 命令在开始或续跑回填时若发现增量队列仍有正在处理的认领，会直接拒绝执行。也正因为要暂停，它是较慢的路径，只在源结构版本的 `_source` 不完整、或新映射需要旧索引从未保存过的数据时才会用到。
