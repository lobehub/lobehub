...
保持持续调度，并反复运行 `bun run search:reindex -- --status`，直到 `pending`、`ready`、`retrying`、`inFlight`、`dead` 和 `revisionLag` 全部为 0。切换查询前必须再检查一次。确认追平后，把 `+search_backend` 追加到现有 `FEATURE_FLAGS` 值并重新部署。追平前不要打开这个开关。Elasticsearch 出错时会直接暴露给调用方，搜索路径不会静默回退到 PostgreSQL 或 `ilike`。
