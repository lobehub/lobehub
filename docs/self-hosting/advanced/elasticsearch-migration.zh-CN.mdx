...
工具把升级路径记录在一份结构版本日志中：`packages/database/src/repositories/ftsSearchDocument/mappings.ts` 里的 `FTS_SEARCH_INDEX_SCHEMA_HISTORY`。每个结构版本对应一条记录，标注该版本的 `_source` 是否完整保存了全部映射字段（`sourceComplete`），以及把处于该版本的部署继续推进所使用的升级策略（`upgrade`：`copy`、`backfill`，或代表最新版本的 `current`）。映射测试会校验这份日志保持连续，且只有 `sourceComplete` 为真的版本才能声明 `copy` 策略。升级 `FTS_SEARCH_INDEX_SCHEMA_VERSION` 并追加这条记录，就是完整的迁移定义；详见下文的[新增结构版本](#新增结构版本)。
...
升级 `FTS_SEARCH_INDEX_SCHEMA_VERSION` 时，需要在 `FTS_SEARCH_INDEX_SCHEMA_HISTORY` 中为新版本追加一条记录，并设置*前一个*版本的 `upgrade` 策略：只有前一个版本的 `_source` 已经保存了新映射所需的全部字段时才能选择 `copy`，否则应选择 `backfill`。之后运行映射测试，它会校验这份日志保持连续，且只有 `sourceComplete` 为真的版本才能声明 `copy`。这条日志记录就是完整的迁移文件，不需要另外编写迁移代码。
