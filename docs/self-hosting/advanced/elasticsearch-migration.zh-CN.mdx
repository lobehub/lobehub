...
<Callout type="warning">
  全量回填命令不会把产品搜索请求切到 Elasticsearch。回填和验收期间，应用必须继续使用 PostgreSQL 搜索。常规数据库迁移会创建 PostgreSQL 增量队列的基础结构和由数据库结构管理的 Memory 扇出 GIN 索引，但不会安装搜索变更采集函数和触发器。只有显式执行 `bun run db:install-search-capture` 或启动 `bun run search:reindex -- --apply` 后才会开始采集，即使增量消费器仍处于暂停状态也是如此。
</Callout>
...
| `ES_INCREMENTAL_SYNC_ENABLED` | 增量同步 | 只有全量回填就绪后才能设为 `true`，用于启用增量消费器。                                    |
| `ES_REINDEX_STATE_DIR`        | 执行时  | 本地 checkpoint 目录；修改状态时必须显式配置，且每次续跑都必须保留并使用同一个目录。                   |
| `SEARCH_BACKEND`              | 运行时  | 查询后端：`pg_search`（默认）或 `elasticsearch`；完成回填、追平和验收前必须保持 `pg_search`。 |
...
5. 在首次回填前安装可选的 PostgreSQL 变更采集：
   ...
   安装器会先严格校验由数据库结构管理的 GIN 索引（不会创建这个索引），再在一个事务中原子安装并验证采集函数和 16 个触发器。完整的预期设施已经存在时可以安全重复执行；如果发现部分对象、被禁用的对象或定义异常，安装器会直接失败，不会静默修复。安装成功后，源数据变更会立即写入增量队列。`bun run search:reindex -- --apply` 也会自动执行这一步；如果希望在准备 Elasticsearch 目标期间就开始采集，可以先显式执行安装命令。若不打算启用 Elasticsearch，不要执行这一步。
...
`ES_INCREMENTAL_SYNC_ENABLED=true`，再运行有明确上限的增量消费器：
...
官方镜像内置 `/app/elasticsearch-sync.cjs`，执行的是同一套有界消费逻辑：
...
保持持续调度，并反复运行 `bun run search:reindex -- --status`，直到 `pending`、`ready`、`retrying`、`inFlight`、`dead` 和 `revisionLag` 全部为 0。切换查询前必须再检查一次。确认追平后，把 `SEARCH_BACKEND` 设为 `elasticsearch` 并重新部署。追平前必须保持 `SEARCH_BACKEND=pg_search`。Elasticsearch 出错时会直接暴露给调用方，搜索路径不会静默回退到 PostgreSQL 或 `ilike`。
...
<Callout type="info">
  全量回填命令不会代替持续运行的增量消费器，也不会自行宣称应用已经可以切换搜索后端。只要 Elasticsearch 仍在承担搜索，就必须持续调度 `search:sync`。
</Callout>
