...
- 升级结构版本时会复用同一套可续跑的回填机制，写入带新版本号后缀的物理索引，并可使用 `--switch-aliases` 参数完成原子切换，详见[结构版本升级](#结构版本升级)。
...
回填工具支持在线、不停机地升级搜索索引结构版本（例如从 `v1` 升级到 `v2`）：使用与首次迁移相同的可续跑回填机制，再加上一个 `--switch-aliases` 参数，用于把全部稳定别名原子切换到新的物理索引。`--switch-aliases` 只能与 `--apply --yes` 一起使用。默认回填行为保持不变：不传 `--switch-aliases` 时，如果稳定别名已经指向另一个物理索引，命令仍会直接失败。增量队列存在尚未处理完的任务时，命令也会拒绝 `--switch-aliases`；请先暂停增量消费器。
...
1. 暂停增量消费器（设置 `FTS_SEARCH_SYNC_ENABLED=false`，或停止 `fts-search:sync` 进程）。产品搜索继续使用当前的稳定别名提供服务，增量队列会持续积累变更，不会丢失数据。

2. 向新结构版本的物理索引开始一次全新回填：

   ```bash
   bun run fts-search:reindex -- --apply --fresh-run --yes
   ```

   结构版本升级必须使用新的 checkpoint（`--fresh-run`），因为它会写入带新版本号后缀的物理索引（例如 `-v2`），并基于通过写入围栏预留的基准修订号，使用 `external_gte` 版本机制写入；这次执行和首次迁移一样可以安全续跑。

3. 回填状态变为 `ready_for_incremental_sync` 后，执行原子切换：

   ```bash
   bun run fts-search:reindex -- --apply --switch-aliases --yes
   ```

   这会在一次原子 `_aliases` 请求中，把全部稳定别名从旧物理索引切换到新物理索引。如果在第 2 步就带上 `--switch-aliases`，回填一完成就会自动执行同样的切换，不需要再单独执行这一步。

4. 恢复增量消费器。增量队列中待处理的记录会重放写入新结构版本的索引；早于回填已经覆盖范围的记录会被 Elasticsearch 判定为版本冲突（409）拒绝，消费器会把这种情况视为成功。

5. 确认搜索正常后，手动删除旧的物理索引，例如：
...
如果需要在第 4 步之前回滚，可以手动通过 `_aliases` 把稳定别名指回旧的物理索引，再恢复增量消费器。
