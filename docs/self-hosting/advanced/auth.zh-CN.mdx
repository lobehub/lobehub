...
| `EMAIL_SERVICE_PROVIDER`  | 可选 | 邮件服务选择：`nodemailer`（默认，SMTP）或 `resend`                                                    | `resend`                         |
| `SMTP_HOST`               | 必选 | SMTP 服务器主机名，仅在 `EMAIL_SERVICE_PROVIDER=nodemailer` 时需要                                    | `smtp.gmail.com`                 |
| `SMTP_PORT`               | 必选 | SMTP 服务器端口（TLS 通常为 `587`，SSL 为 `465`），仅在 `EMAIL_SERVICE_PROVIDER=nodemailer` 时需要          | `587`                            |
| `SMTP_SECURE`             | 可选 | SSL 设置为 `true`（端口 465），TLS 设置为 `false`（端口 587），仅在 `EMAIL_SERVICE_PROVIDER=nodemailer` 时需要 | `false`                          |
| `SMTP_USER`               | 必选 | SMTP 认证用户名，仅在 `EMAIL_SERVICE_PROVIDER=nodemailer` 时需要                                     | `user@gmail.com`                 |
| `SMTP_PASS`               | 必选 | SMTP 认证密码，仅在 `EMAIL_SERVICE_PROVIDER=nodemailer` 时需要                                      | `your-app-password`              |
| `RESEND_API_KEY`          | 必选 | Resend API Key，`EMAIL_SERVICE_PROVIDER=resend` 时必填                                        | `re_xxxxxxxxxxxxxxxxxxxxxx`      |
| `RESEND_FROM`             | 推荐 | 默认发件人地址，需为 Resend 已验证域名下的邮箱，`EMAIL_SERVICE_PROVIDER=resend` 时使用                           | `noreply@your-domain.com`        |
