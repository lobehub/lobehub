...
- 类型：可选
- 描述：控制实例的生命周期。`persistent` 为每个话题保留一个容器并在到期前续期，只要话题还活跃，变量和文件等状态就一直保留；`on-demand` 不续期，容器在 `TENCENT_SANDBOX_TIMEOUT_SEC` 后自然失效，以此约束闲置成本，代价是空闲一段时间后会有冷启动。两种模式都会向腾讯控制面发送不含用户 / 话题原始 ID 的稳定会话标识，因此跨服务副本、冷启动以及构成同一次工具调用的多个请求，都能重新连接同一个尚未过期的容器。
- 默认值：`persistent`
- 示例：`on-demand`
