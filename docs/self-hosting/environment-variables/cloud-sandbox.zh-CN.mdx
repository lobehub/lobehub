...
- 类型：可选
- 描述：控制实例的生命周期。`persistent` 为每个话题保留一个容器并在到期前续期，只要话题持续活跃，变量和文件等状态就一直保留；`on-demand` 不续期，容器在 `TENCENT_SANDBOX_TIMEOUT_SEC` 后自然失效，以此约束实例存活时长，代价是超过配置的生命周期后会有冷启动。两种模式都会向腾讯控制面发送不含用户 / 话题原始 ID 的稳定会话标识，因此跨服务副本、冷启动以及构成同一次工具调用的多个请求，都能重新连接同一个尚未过期的容器。
- 默认值：`persistent`
- 示例：`on-demand`
...
- 类型：可选
- 描述：每个沙箱实例申请的存活时长（秒），有效范围为 300–3600。`persistent` 模式下会在到期前自动续期。
- 默认值：`300`
- 示例：`900`
