...
To centrally select the provider and model used by the default agent, configure both fields. The provider value is the provider ID used by LobeHub:

```env
DEFAULT_AGENT_CONFIG='provider=xiaomimimo;model=mimo-v2.5-pro'
```

This works with any supported provider. Make sure the corresponding server-side provider is enabled (for example, set `XIAOMIMIMO_API_KEY`) and that the selected model is not excluded by its `*_MODEL_LIST` configuration. `DEFAULT_AGENT_CONFIG` does not verify provider or model availability, so an unavailable selection cannot serve chat requests.
