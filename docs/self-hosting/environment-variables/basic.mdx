...
- Type: Optional
- Description: Comma-separated image formats accepted by the fallback multimodal understanding model. Supported values are `jpeg`, `png`, `webp`, or their MIME types. Images in other formats are detected from their actual bytes and converted to the first configured format before being sent to the model.
- Default: `jpeg,png`
