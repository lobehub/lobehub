...
To provide the fallback provider and model for the default agent, configure both fields. The provider value is the provider ID used by LobeHub:

```env
DEFAULT_AGENT_CONFIG='provider=xiaomimimo;model=mimo-v2.5-pro'
```

This value is merged first as an initial default. A user's default settings and an agent's persisted configuration take precedence, so changing the environment variable does not override existing agents that already specify a provider or model. It applies when no more specific selection is stored.

This works with any supported provider. Make sure the corresponding server-side provider is enabled (for example, set `XIAOMIMIMO_API_KEY`) and that the selected model is not excluded by its `*_MODEL_LIST` configuration. `DEFAULT_AGENT_CONFIG` does not verify provider or model availability, so an unavailable fallback cannot serve chat requests.
