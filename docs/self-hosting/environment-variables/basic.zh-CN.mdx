...
如需由管理员统一指定默认助理使用的服务商和模型，请同时配置这两个字段。服务商的值应使用 LobeHub 内部的 provider ID：

```env
DEFAULT_AGENT_CONFIG='provider=xiaomimimo;model=mimo-v2.5-pro'
```

此配置适用于任意受支持的模型服务商。请确保对应的服务端模型服务商已经启用（例如设置 `XIAOMIMIMO_API_KEY`），并且所选模型没有被对应的 `*_MODEL_LIST` 配置排除。`DEFAULT_AGENT_CONFIG` 不会校验服务商或模型是否可用，因此不可用的配置无法正常发起对话请求。
