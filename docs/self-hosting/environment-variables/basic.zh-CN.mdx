...
如需为默认助理提供回退服务商和模型，请同时配置这两个字段。服务商的值应使用 LobeHub 内部的 provider ID：

```env
DEFAULT_AGENT_CONFIG='provider=xiaomimimo;model=mimo-v2.5-pro'
```

此值会最先作为初始默认配置合并。用户的默认设置和 Agent 已持久化的配置具有更高优先级，因此修改该环境变量不会覆盖已经指定服务商或模型的现有 Agent。只有在没有保存更具体的选择时，才会使用此回退配置。

此配置适用于任意受支持的模型服务商。请确保对应的服务端模型服务商已经启用（例如设置 `XIAOMIMIMO_API_KEY`），并且所选模型没有被对应的 `*_MODEL_LIST` 配置排除。`DEFAULT_AGENT_CONFIG` 不会校验服务商或模型是否可用，因此不可用的回退配置无法正常发起对话请求。
