...
- 类型：必选
- 描述：这是你在 Hubris 控制台申请的 API 密钥，可以在[这里](https://hubris.pw/keys)创建
- 默认值：-
- 示例：`sk-gw-xxxxxx...xxxxxx`

### `HUBRIS_MODEL_LIST`

- 类型：可选
- 描述：用来控制 Hubris 模型列表，使用 `+` 增加一个模型，使用 `-` 来隐藏一个模型，使用 `模型名=展示名` 来自定义模型的展示名，用英文逗号隔开。模型定义语法规则见 [模型列表][model-list]
- 默认值：`-`
- 示例：`-all,+anthropic/claude-sonnet-5,+google/gemini-3.8-flash`

### `HUBRIS_PROXY_URL`

- 类型：可选
- 描述：用于覆盖 Hubris 的 API 基础地址，仅在通过自建网关转发请求时才需要设置
- 默认值：`https://api.hubris.pw/v1`
- 示例：`https://api.hubris.pw/v1`
