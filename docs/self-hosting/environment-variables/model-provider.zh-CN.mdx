...
- 类型：必选
- 描述：这是你在 TokenLab 服务中申请的 API 密钥
- 默认值：-
- 示例：`sk-xxxxxx...xxxxxx`
...
- 类型：可选
- 描述：TokenLab API 的基础地址。TokenLab 在同一个网关中支持 OpenAI-compatible chat、Responses API、Anthropic Messages API、Gemini-native generateContent、embeddings、images、videos、audio、realtime 和 translations 等入口。
- 默认值：`https://api.tokenlab.sh/v1`
- 示例：`https://api.tokenlab.sh/v1`

### `TOKENLAB_MODEL_LIST`

- 类型：可选
- 描述：用来控制 TokenLab 模型列表，使用 `+` 增加一个模型，使用 `-` 来隐藏一个模型，使用 `模型名=展示名` 来自定义模型的展示名，用英文逗号隔开。模型定义语法规则与其他 provider 保持一致。
- 默认值：`-`
- 示例：`-all,+gpt-5.5,+claude-opus-4-8=Claude Opus 4.8`
