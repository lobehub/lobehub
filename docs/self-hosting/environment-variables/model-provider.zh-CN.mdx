...
- 类型：可选
- 描述：访问小米 MiMo 时使用的基础 URL。如需使用代理或自定义 OpenAI 兼容端点，请配置此选项
...
- 类型：可选
- 描述：用来控制小米 MiMo 模型列表，使用 `+` 增加一个模型，使用 `-` 隐藏一个模型，使用 `模型名=展示名` 自定义模型的展示名，用英文逗号隔开。模型定义语法规则与其他 provider 保持一致
- 默认值：`-`
- 示例：`-all,+mimo-v2.5-pro,+mimo-v2.5=MiMo V2.5,+mimo-v2-flash=MiMo V2 Flash`
