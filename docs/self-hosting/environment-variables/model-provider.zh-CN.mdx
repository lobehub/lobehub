...
连接 [Cheaper Inference](https://cheaperinference.com) 的 OpenAI 兼容 API，设置步骤见[在 LobeHub 中使用 Cheaper Inference](/zh/docs/usage/providers/cheaperinference)。

### `CHEAPERINFERENCE_API_KEY`

- 类型：部署时可选；提供共享服务端密钥时必填。用户也可以在服务商设置中填写自己的密钥。
- 描述：在 [Cheaper Inference 账户](https://cheaperinference.com/signup) 中创建的密钥。设置非空密钥后，服务端配置会启用 Cheaper Inference。LobeHub 根据服务商 ID 派生变量名，因此使用 `CHEAPERINFERENCE_API_KEY`，与 Cheaper Inference 文档中的 `CHEAPER_INFERENCE_API_KEY` 名称不同。
- 默认值：未设置，不提供共享服务端密钥。
- 示例：`ci_live_xxxxxx...xxxxxx`

### `CHEAPERINFERENCE_PROXY_URL`

- 类型：可选
- 描述：覆盖 OpenAI 兼容 API 的基础地址。代理需要支持 `/chat/completions` 和 `/responses`。请填写基础地址，而不是完整的接口地址。
- 默认值：`https://api.cheaperinference.com/v1`
- 示例：`https://your-proxy.example.com/v1`

### `CHEAPERINFERENCE_MODEL_LIST`

- 类型：可选
- 描述：使用通用的[模型列表语法][model-list]自定义模型列表：`+` 增加模型，`-` 隐藏模型，`模型ID=展示名` 自定义展示名，用英文逗号隔开。
- 默认值：未设置，使用内置模型列表。
