...
- Type: Optional
- Description: Controls the instance lifecycle. `persistent` keeps one container per topic and renews it before expiry, so state such as variables and files survives while the topic remains active. `on-demand` lets the container lapse when `TENCENT_SANDBOX_TIMEOUT_SEC` elapses instead of renewing it, which bounds lifetime at the price of a cold start after the configured lifetime. Both modes use an opaque, deterministic conversation ID in Tencent's control plane, so the same live container is reacquired across server replicas, cold starts, and all calls that make up a tool invocation.
- Default: `persistent`
- Example: `on-demand`
...
- Type: Optional
- Description: Lifetime requested for each sandbox instance, in seconds. Valid values are 300–3600. In `persistent` mode the instance is renewed before this expires.
- Default: `300`
- Example: `900`
