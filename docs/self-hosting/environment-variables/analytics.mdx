...
The integration uses the Langfuse JavaScript SDK v5 and supports Langfuse v4. `ENABLE_LANGFUSE` works independently of `ENABLE_TELEMETRY`; users must still opt in to chat telemetry. Chat completions are recorded as a root observation with a nested generation. Message edits create new events with before/after content instead of overwriting completed observations.
