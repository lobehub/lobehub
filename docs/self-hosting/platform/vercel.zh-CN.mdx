...
<Steps>
  ...
  每次 `main` 分支更新后，该工作流都会自动构建并部署；仓库内置的 **Upstream Sync** 工作流成功完成后也会触发部署。必须补充这一触发条件，是因为[使用 `GITHUB_TOKEN` 推送的提交不会启动另一个工作流](https://docs.github.com/zh-cn/actions/how-tos/write-workflows/choose-when-workflows-run/trigger-a-workflow#triggering-a-workflow-from-a-workflow)。你也可以在 **Actions → Vercel Prebuilt Deploy → Run workflow** 中手动触发。
