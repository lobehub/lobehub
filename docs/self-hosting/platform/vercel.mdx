...
<Steps>
  ...
  The workflow builds and deploys whenever `main` is updated. It also deploys after the repository's built-in **Upstream Sync** workflow completes successfully. This extra trigger is required because [pushes made with `GITHUB_TOKEN` do not start another workflow run](https://docs.github.com/en/actions/how-tos/write-workflows/choose-when-workflows-run/trigger-a-workflow#triggering-a-workflow-from-a-workflow). You can also trigger it manually from **Actions → Vercel Prebuilt Deploy → Run workflow**.
