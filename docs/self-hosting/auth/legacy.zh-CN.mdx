...
<Callout type={'warning'}>
  **旧版提示**：NextAuth 和 Clerk 已从 LobeHub 中完全移除。配置下文相关环境变量会导致部署在构建 / 启动阶段失败。本页仅供历史迁移参考，请改用 [Better Auth](/zh/docs/self-hosting/auth)。
</Callout>

以下认证方式（NextAuth、Clerk）已从 LobeHub 移除，配置相关变量会导致部署失败，本页面仅供历史迁移参考。
