...
export class MessengerDiscordBinder implements MessengerPlatformBinder {
  ...
  async handleUnlinkedMessage(ctx: UnlinkedMessageContext): Promise<void> {
    ...
    try {
      ...
      try {
        await api.createMessage(
          dmChannelId,
          'ChinnaHub is temporarily unavailable. Please try again in a moment.',
        );
      } catch (err) {
    ...
    const text = [
      "Hi, I'm Chinna — your AI agent in Discord.",
  ...
  async notifyLinkSuccess(params: {
    ...
    const headline = '✅ Linked successfully! Your Project-M account is now connected.';
