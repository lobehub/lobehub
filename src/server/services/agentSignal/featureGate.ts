...
export interface AgentSelfIterationCapabilityInput {
  ...
  /**
   * Whether the agent is the product-owned Chinna agent.
   */
  isLobeAiAgent: boolean;
}

/**
 * Checks whether a slug belongs to Chinna.
 *
 * Use when:
 * - Agent Signal must special-case the product-owned default assistant
 * - Code must avoid treating every virtual/builtin agent as Chinna
 *
 * Expects:
 ...
 * - Chinna should be managed by feature flag instead of stored chat config
 *
 * Expects:
 * - `isAgentSelfIterationFeatureEnabled` already includes the user-level feature-flag result
 *
 * Returns:
 * - `true` for Chinna when the feature flag is enabled
 * - `true` for non-Chinna only when `agentSelfIterationEnabled` is true
 */
