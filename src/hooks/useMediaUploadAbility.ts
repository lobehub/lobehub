...
export const useMediaUploadAbility = (model: string, provider: string, agentId?: string) => {
  ...
  const [bypassMediaGate, heterogeneousAgentType] = useAgentValue(
    agentId,
    (agent) =>
      [
        Boolean(agentId) &&
          (agentProjectionSelectors.enableMode(agent) ||
            agentProjectionSelectors.heterogeneous(agent)),
        agentProjectionSelectors.heterogeneousProviderType(agent),
      ] as const,
  );
