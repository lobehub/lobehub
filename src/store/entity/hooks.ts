...
import type { HomeDailyBriefResponse, HomeDataRequestMarker } from '@lobechat/types';
...
export const useHomeRecentTopicsRequest = (
  ...
  const request = useClientDataSWR<HomeDataRequestMarker>(
    isLogin ? entityDataKeys.recentTopics(scope, limit) : null,
    async () => {
      const observedAt = Date.now();
      const items = await recentService.getAll(limit, ['topic']);
      getEntityStoreState().ingestHomeRecentTopics(scope, items, limit, observedAt);
      return marker(observedAt);
    },
    { revalidateOnFocus: false },
  );
