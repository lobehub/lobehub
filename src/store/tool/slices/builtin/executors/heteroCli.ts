 ...
 * (`amp` / `claude-code` / `codebuddy` / `codex` / `kimi-code` / `opencode` / `pi` /
...
// AMP's shell tool is `shell_command`; CC, CodeBuddy, and Qoder use `Bash`;
// Kimi Code uses `Shell`; Codex uses `command_execution`; OpenCode and Pi use `bash`.
