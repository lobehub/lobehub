  ...
  describe('createAssistantMessage', () => {
    ...
    describe('should handle content correctly for vision models', () => {
      it('should include image content when with vision model', async () => {
        ...
        const { isLocalOrPrivateUrl } = await import('@lobechat/utils/url');
        vi.mocked(parseDataUri).mockReturnValue({ type: 'url', base64: null, mimeType: null });
        vi.mocked(isLocalOrPrivateUrl).mockReturnValue(false); // Not a local URL
...
<!-- END SYSTEM CONTEXT -->`,
    ...
    describe('local image URL conversion', () => {
      it('should convert local image URLs to base64 and call processImageList', async () => {
        ...
        // Mock for local URL
        vi.mocked(parseDataUri).mockReturnValue({ type: 'url', base64: null, mimeType: null });
        vi.mocked(isLocalOrPrivateUrl).mockReturnValue(true); // This is a local URL
        ...
        expect(isLocalOrPrivateUrl).toHaveBeenCalledWith('http://127.0.0.1:3000/uploads/image.png');
...
<!-- END SYSTEM CONTEXT -->`,
      ...
      it('should not convert remote URLs to base64 and call processImageList', async () => {
        ...
        // Mock for remote URL
        vi.mocked(parseDataUri).mockReturnValue({ type: 'url', base64: null, mimeType: null });
        vi.mocked(isLocalOrPrivateUrl).mockReturnValue(false); // This is NOT a local URL
        ...
        expect(isLocalOrPrivateUrl).toHaveBeenCalledWith('https://example.com/remote-image.jpg');
...
<!-- END SYSTEM CONTEXT -->`,
      ...
      it('should handle mixed local and remote URLs correctly', async () => {
        ...
        // Mock isLocalOrPrivateUrl to return true only for 127.0.0.1 URLs
        vi.mocked(isLocalOrPrivateUrl).mockImplementation((url: string) => {
        ...
        // Verify isLocalOrPrivateUrl was called for each image
        expect(isLocalOrPrivateUrl).toHaveBeenCalledWith('http://127.0.0.1:3000/local1.jpg');
        expect(isLocalOrPrivateUrl).toHaveBeenCalledWith('https://example.com/remote1.png');
        expect(isLocalOrPrivateUrl).toHaveBeenCalledWith('http://127.0.0.1:8080/local2.gif');
