...
const AskAgentCommands = memo(() => {
  ...
  // Check if Chinna matches the query
  const showLobeAI = !mentionQuery || 'Chinna AI'.includes(mentionQuery);
    ...
    <Command.Group heading={t('cmdk.mentionAgent')}>
      ...
      {showLobeAI && (
        <Command.Item
          ...
          onSelect={() => handleAgentSelect(inboxAgentId, 'Chinna AI', DEFAULT_INBOX_AVATAR)}
        >
          <Avatar emojiScaleWithBackground avatar={DEFAULT_INBOX_AVATAR} shape="square" size={18} />
          <div className={styles.itemContent}>
            <div className={styles.itemLabel}>@Chinna</div>
          </div>
        </Command.Item>
      )}
