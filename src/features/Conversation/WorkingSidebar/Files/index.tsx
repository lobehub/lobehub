...
interface FilesProps {
  ...
  deviceId?: string;
  /**
   * Which instance's directory the tree shows. The address, and the reason a
   * tree can be shown before the conversation exists — a device's tree needs
   ...
   * and it is absent until the conversation exists.
   */
  ...
  ({ deviceId, sandboxInstanceId, sandboxTopicId, workingDirectory }) => {
    ...
    const openNode = useCallback(
      (node: ExplorerTreeNode<ProjectFileIndexEntry>) => {
        ...
        // The viewer reads a sandbox file through the topic's session, so there
        // is nothing to open before the conversation exists. The tree is still
        // worth showing — seeing what an instance holds is most of the value —
        // and doing nothing here is better than opening a pane that cannot load.
        if (isSandbox && !sandboxTopicId) return;
      ...
      [deviceId, isRemote, isSandbox, openLocalFile, projectRoot, sandboxTopicId],
    );
