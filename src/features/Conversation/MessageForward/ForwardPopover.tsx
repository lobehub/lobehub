...
import { Flexbox, Popover, SearchBar, Text, TextArea } from '@lobehub/ui';
...
/**
 * Hover popover on the Forward button: search + multi-select agents, attach an
 * optional note, and submit. Replaces the old full-screen forward dialog.
 */
const ForwardPopover = memo<ForwardPopoverProps>(({ children, disabled, open, onOpenChange }) => {
  ...
  // Reset the picker each time it closes so the next open starts clean.
  useEffect(() => {
    if (!open) {
      setKeyword('');
      setSelectedIds([]);
...
ForwardPopover.displayName = 'ForwardPopover';
