...
const seedAgentMap = (agentId: string, agentMeta: SharedAgentIdentity['agentMeta']) => {
  ...
  // skeletons. Merged via the store's dispatcher (the `/share/t` precedent)
  // so nulls never clobber existing fields.
...
/**
 * Seeds the agent/chat stores for the visitor-facing share surface (popup
 * quick-chat pattern) and reports whether the one-time destructive init has
 * landed yet.
 *
 ...
 *   the header avatar/name/title stay fresh. SWR revalidation (e.g. the
 *   composer's budget-status "Retry", see `useBudgetStatusRetry`) hands back
 *   a brand-new `agentMeta` object even when the identity hasn't changed;
 *   folding this into the identity effect would wipe `activeTopicId` on
 *   every metadata refresh and yank the visitor into a new conversation
 *   right when a budget block lifts.
 */
