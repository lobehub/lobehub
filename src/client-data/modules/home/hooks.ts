...
import type { ClientDataRequestMarker, HomeDailyBriefResponse } from '@lobechat/types';
...
export const useHomeRecentTopicsRequest = (
  ...
  const scope = useCacheScope();
  const isInitialized = useClientDataStore((state) =>
    Boolean(selectHomeRecentTopics(state.scopes[scope], limit)),
  );

  const request = useClientDataSWR<ClientDataRequestMarker>(
    isLogin ? clientDataKeys.recentTopics(scope, limit) : null,
    async () => {
      const observedAt = Date.now();
      const items = await recentService.getAll(limit, ['topic']);
      getClientDataStoreState().ingestHomeRecentTopics(scope, items, limit, observedAt);
      return marker(observedAt);
    },
    { revalidateOnFocus: false },
  );
