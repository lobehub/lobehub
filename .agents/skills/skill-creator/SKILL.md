...
4. **Launch the viewer** with both qualitative outputs and quantitative data:
   ...
   nohup python \
     --skill-name "my-skill" \
     --benchmark < skill-creator-path > /eval-viewer/generate_review.py \
...
python -m scripts.run_loop \
  --eval-set \
  5 \
  --verbose < path-to-trigger-eval.json > --skill-path < path-to-skill > --model < model-id-powering-this-session > --max-iterations
```
