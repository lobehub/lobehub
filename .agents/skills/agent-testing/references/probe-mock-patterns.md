...
### D15. A visible-looking sidebar link can be off-canvas and fail strict click hit-testing

- **Situation**: `agent-browser click` reports that a sidebar link is covered by the main content
  even though the link still appears in the accessibility snapshot. Repeating the click on the
  link's inner text node fails the same way.
- **Doesn't work**: treating the snapshot entry as proof that the link is on-screen, or forcing a
  programmatic click before measuring its geometry.
- **Works**: inspect `getBoundingClientRect()` and `document.elementFromPoint()` at the link center.
  In the measured case the link started at a negative x coordinate and its center landed under the
  main panel because the sidebar had been collapsed. Re-open the sidebar, verify the link center is
  inside the viewport, refresh element refs, and then use the normal click. Keep the explanation at
  the observed layout level unless the owning component source establishes a deeper cause.

### C6. A fetch logger can contaminate a new phase when it resolves into a mutable global sink

- **Situation**: a test clears `window.__FETCH_LOG` between phases, then a request that started in
  the previous phase settles and appends into the newly assigned array. The new phase falsely appears
  to have initiated that request.
- **Doesn't work**: reading `window.__FETCH_LOG` inside the promise's `then` / `catch`, because that
  looks up whichever array is current at settlement time rather than the phase that owned the start.
- **Works**: capture both the sink and a run id synchronously when `fetch` begins, then append to that
  captured sink on settlement. Count only entries whose start-time run id matches the asserted phase.
  When freshness matters, pair this with the feature's own runtime freshness probe so the precondition
  is measured rather than inferred from timing.

### D16. Verify animated evidence in a browser, and force full-frame GIF tiles when deltas turn black

- **Situation**: a PNG-frame sequence is correct, but the synthesized GIF shows large black rectangles
  in later frames. GIF encoders can write cropped delta tiles whose disposal/transparency behavior does
  not match the intended full screenshot sequence.
- **Doesn't work**: trusting the successful encoder exit code, or repeatedly opening the same `file://`
  URL without a cache-busting query. An immediate screenshot after navigation can also capture the prior
  image before the new GIF finishes decoding.
- **Works**: keep the original PNG frames and inspect them first. If the GIF uses cropped tiles, force the
  delta bounding box to span the full canvas (for example, alternate two imperceptible 1px corner markers
  on opposite corners), then write non-optimized frames. Confirm every GIF tile has full-canvas extents.
  Finally open the GIF in Chromium with a fresh query string, wait briefly before the first capture, and
  inspect both an early frame and a near-terminal frame as the report viewer will render them.

### E16. In zsh, assigning to lowercase `path` rewrites `PATH`

- **Situation**: a validation loop uses `while read -r path; do ...; done`, and every command after the
  loop fails with `command not found` even though the same tools worked earlier in the shell.
- **Cause (confirmed)**: zsh exposes the special lowercase array parameter `path`, tied to the scalar
  environment variable `PATH`. Assigning an evidence filename to `path` replaces the executable search
  path for that shell.
- **Works**: never use `path` as a local/loop variable in zsh scripts. Use a descriptive name such as
  `evidence_path` or declare a safely named local. A new shell starts with the original environment; no
  system-wide repair is needed.

### E17. Successful `verify ingest-report` may not write the reuse sidecar

- **Situation**: publishing the same report directory twice creates two verify runs even though the
  workflow expects `.verify-run.json` to make the second ingest update the first one.
- **Observed**: both ingests exited successfully and returned a run id, but the report directory had no
  `.verify-run.json` after either call. The second call therefore had no persisted target and created a
  new run. Cause inside the CLI was not established during the test session.
- **Works**: inspect the report directory for the sidecar immediately after the first publish. If it is
  absent, preserve the returned run id and check `lh verify ingest-report --help` before updating. The
  installed CLI in the measured run did **not** support the documented `--run` option (`unknown option
'--run'`). On that version, avoid a second aggregate ingest; upgrade to a version with targeted update
  support or use the atomic verify commands that accept an explicit run id. Do not assume directory reuse
  alone is sufficient until the sidecar is present.
