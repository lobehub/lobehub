...
The same bar applies **beyond error states**. Tool and agent result cards are the other place machine vocabulary leaks, because the view renders whatever structured payload the service returned: JSON paths (`$.budget`), snake\_case check keys (`is_valid_restaurants`), raw enum values (`self-driving`), internal pack / provider ids spliced into a sentence ("Solved travelplanner"), and service-assembled English explanations shown verbatim in a zh UI. Map keys to i18n labels at the view (falling back to the raw key only for unknown ones), have the service return structured `{ kind, params }` for explanations the view can localize, and keep machine-facing repair detail (a spec-validation dump meant for the model) behind an inspect / debug affordance.

> ❌ **Constraint solver** result cards (`builtin-tool-solver`): conflicts show `$.budget` and the server's English "budget 100.00 is below the minimum feasible plan cost 306.00" (`InfeasibleResult.tsx:34-40`); verify lists raw `is_reasonable_visiting_city` keys (`Render/Verify.tsx:62-64`); constraint tags are hardcoded English (`CandidateList.tsx:99-104`); the Inspector reads "已完成 travelplanner 求解" from `'Solved {{pack}}'`. See `ux-audit/references/example/solver-tool.md`.

- [ ] Non-error rendered states (tool / result cards) show no JSON paths, snake\_case keys, raw enum values or internal ids — keys map to localized labels, and service-assembled explanations are localized from structured params. _(Meaningful・Certainty)_
