...
compact abbreviated value and **advance the unit at every 1000× boundary** — never
let a magnitude pile up inside a smaller unit. `10285.7M` is a bug: past 1000M the
value should read `10.3B`. A number that keeps growing digits in one unit (`9092.9M`,
`10285.7M`) forces the reader to count zeros and defeats the point of abbreviating.

- [ ] **Roll the unit at 1000×, don't overflow it.** Use K → M → B → T, switching
      the moment the value reaches the next threshold (≥ 1000K → M, ≥ 1000M → B, …).
      A displayed number should never carry a coefficient ≥ 1000. ✅ `10.3B` — ❌
      `10285.7M`. _(Certainty)_
- [ ] **Keep the precision compact.** One decimal, drop a trailing `.0` (`1M`, not
      `1.0M`; `9.1B`, not `9.09B`). The abbreviation is a glance value; full precision
      belongs in a tooltip / detail row. _(Natural)_
- [ ] **Use the shared helper, don't re-roll it.** Token/usage counts go through
      `formatUsageValue` (`@lobechat/utils`); general shortenings through
      `formatShortenNumber`. Both already carry the K/M/B/T ladder — reach for them
      instead of an ad-hoc `(n / 1_000_000).toFixed(1) + 'M'` that stops at M. _(Certainty)_
