...
Read the FAQ/Troubleshooting section from the migration documentation:

For NextAuth migration:

```bash
cat docs/self-hosting/advanced/auth/nextauth-to-betterauth.mdx | grep -A 500 "## FAQ\|## Troubleshooting\|## Common Issues"
```

For Clerk migration:

```bash
cat docs/self-hosting/advanced/auth/clerk-to-betterauth.mdx | grep -A 500 "## FAQ\|## Troubleshooting\|## Common Issues"
```

Match the user's issue against documented solutions. If a solution exists in the docs, provide it with a link to the documentation.
