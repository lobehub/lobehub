...
const authHtmlFallbackPath = resolve(root, 'dist/auth/index.html');
const authHtmlSourcePath = existsSync(authHtmlPath)
  ? authHtmlPath
  : existsSync(authHtmlFallbackPath)
    ? authHtmlFallbackPath
    : undefined;

if (authHtmlSourcePath) {
  const authHtml = readFileSync(authHtmlSourcePath, 'utf8');
