...
import { normalizeSpaHtmlPublicPaths } from './generateSpaTemplatesCore.ts';

const root = path.resolve(import.meta.dirname, '..');

const desktopHtml = normalizeSpaHtmlPublicPaths(
  readFileSync(path.resolve(root, 'dist/desktop/index.html'), 'utf8'),
);
...
if (hasMobileBuild) {
  ...
  const mobileHtml = normalizeSpaHtmlPublicPaths(
    readFileSync(existsSync(mobileHtmlPath) ? mobileHtmlPath : mobileHtmlFallbackPath, 'utf8'),
  );
...
if (existsSync(authHtmlPath)) {
  const authHtml = normalizeSpaHtmlPublicPaths(readFileSync(authHtmlPath, 'utf8'));
