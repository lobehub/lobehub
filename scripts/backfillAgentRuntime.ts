...
export interface AgentRuntimeRow {
  id: string;
  runtimeKind: 'heterogeneous' | 'native';
  runtimeType: string | null;
  storedRuntimeKind: string;
  storedRuntimeType: string | null;
}

export const getAgentRuntimeBackfillBatchQuery = (lockRows: boolean) => `
  SELECT
    id,
    ...
    while (true) {
      ...
      try {
        ...
        const result = await client.query<AgentRuntimeRow>(
          getAgentRuntimeBackfillBatchQuery(apply),
          [cursor, batchSize, HETEROGENEOUS_AGENT_MODEL_IDS],
        );
