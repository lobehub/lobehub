...
const mockTopupChunk = bill.split("mockTopup")[1]?.slice(0, 800) || "";
const mockOrgChunk = orgR.split("mockOrgTopup")[1]?.slice(0, 800) || "";

const probes = {
  managementSilentMock: /AICO_OPENROUTER_MOCK \|\| !aicoEnv.OPENROUTER_MANAGEMENT_API_KEY/.test(mgmt),
  migration0132HasDefault: /user_id" text NOT NULL DEFAULT/i.test(mig),
  mockOrgHasProdGate: /AICO_ALLOW_MOCK|NODE_ENV/.test(mockOrgChunk),
  mockTopupHasProdGate: /FORBIDDEN|AICO_ALLOW_MOCK/.test(mockTopupChunk),
  smsDebugFallback: /SmsImplType.Kavenegar : SmsImplType.Debug/.test(sms),
  twinPaths: twin,
  walletUiGated: /AICO_ALLOW_MOCK|isProduction/.test(wallet),
};
...
const journal = JSON.parse(fs.readFileSync("packages/database/migrations/meta/_journal.json", "utf8"));
