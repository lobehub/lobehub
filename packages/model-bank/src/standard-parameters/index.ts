...
/**
 * Ids synthesized by `postProcessModelList` for the provider Image tab
 * (`{baseId}:image`). Detection is suffix-based so modality-driven clones
 * (not only the Gemini whitelist) stay dedupe-safe.
 */
export const isAutoGeneratedImageModelId = (modelId: string) =>
  modelId.endsWith(IMAGE_MODEL_SUFFIX);
