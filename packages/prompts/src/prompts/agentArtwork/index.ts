...
const STYLE_DIRECTIONS: Record<AgentArtworkStyle, string> = {
  ...
  lobe: "Render it as a single 3D cartoon character in the spirit of a collectible designer figure: smooth rounded shapes, expressive finely-crafted features with big lively eyes, balanced natural proportions (not an oversized baby head), soft studio lighting, and one vivid saturated solid background color. Frame it as an upper-body bust portrait cropped around the chest — head and shoulders fill most of the frame, arms mostly out of frame, no waist or legs visible. Express the identity through the character's outfit and accessories — do not draw scenes, maps, or diagrams on the character's body.",
  pixel:
