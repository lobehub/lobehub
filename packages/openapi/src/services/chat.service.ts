...
export class ChatService extends BaseService {
  ...
  private async getApiKey(provider: string) {
    ...
    if (!aiProviderConfigs || aiProviderConfigs.length === 0) {
      this.log('info', 'No valid AI provider config found, falling back to environment variable config', {
        provider,
        userId: this.userId,
      });
    ...
    try {
      ...
      if (params.sessionId) {
        ...
        if (!agentAndModel.length) {
          this.log('warn', 'Model config for the session does not exist', {
            sessionId: params.sessionId,
          });
        ...
        this.log('info', 'Successfully retrieved session model config from database', {
          agentId: agent.id,
          modelId: model.id,
        ...
        if (!agentToSession) {
          throw this.createNotFoundError('Agent for the session does not exist');
        }

        this.log('info', 'Successfully retrieved model config by sessionId', {
          sessionId: params.sessionId,
        });
