...
export class AgentService extends BaseService {
  ...
  async queryAgents(request: GetAgentsRequest): ServiceResult<AgentListResponse> {
    this.log('info', 'Getting Agent list', { request });
  ...
  async createAgent(request: CreateAgentRequest): ServiceResult<AgentDetailResponse> {
    this.log('info', 'Creating Agent', { title: request.title });
  ...
  async updateAgent(request: UpdateAgentRequest): ServiceResult<AgentDetailResponse> {
    this.log('info', 'Updating Agent', { id: request.id, title: request.title });
    ...
    try {
      ...
      if (!permissionResult.isPermitted) {
        throw this.createAuthorizationError(permissionResult.message || 'Unauthorized to update this Agent');
      }
  ...
  async deleteAgent(request: AgentDeleteRequest): ServiceResult<void> {
    ...
    try {
      ...
      if (!permissionResult.isPermitted) {
        throw this.createAuthorizationError(permissionResult.message || 'Unauthorized to delete this Agent');
      }
  ...
  async getAgentById(agentId: string): ServiceResult<AgentDetailResponse | null> {
    this.log('info', 'Getting Agent detail by ID', { agentId });
    ...
    try {
      ...
      if (!permissionResult.isPermitted) {
        throw this.createAuthorizationError(permissionResult.message || 'Unauthorized to access this Agent');
      }

      if (!this.userId) {
        throw this.createAuthError('Not logged in, cannot get Agent detail');
      }
  ...
  private async migrateAgentSessions(fromAgentId: string, toAgentId: string): Promise<void> {
    ...
    try {
      ...
      this.handleServiceError(error, 'migrate sessions');
    }
  }
}
