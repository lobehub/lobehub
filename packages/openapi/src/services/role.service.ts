    ...
    try {
      return await this.db.transaction(async (tx) => {
        ...
        if (existingRole.isSystem && (updateData.name || updateData.isSystem === false)) {
          throw this.createBusinessError('System roles are not allowed to have their name or system attribute modified');
        }
    ...
    try {
      return await this.db.transaction(async (tx) => {
        ...
        if (existingRole.isSystem) {
          throw this.createBusinessError('System roles are not allowed to be deleted');
        }
