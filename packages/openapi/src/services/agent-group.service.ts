...
export class AgentGroupService extends BaseService {
  ...
  async getAgentGroups(): ServiceResult<AgentGroupListResponse> {
    ...
    try {
      ...
      if (!permissionResult.isPermitted) {
        throw this.createAuthorizationError(permissionResult.message || 'Unauthorized to access agent group list');
      }
  ...
  async getAgentGroupById(groupId: string): ServiceResult<AgentGroupListResponse[0] | null> {
    try {
      ...
      const permissionResult = await this.resolveOperationPermission('AGENT_READ');
      if (!permissionResult.isPermitted) {
        throw this.createAuthorizationError(permissionResult.message || 'Unauthorized to access this agent group');
      }
  ...
  async createAgentGroup(request: CreateAgentGroupRequest): ServiceResult<string> {
    ...
    try {
      ...
      if (!permissionResult.isPermitted) {
        throw this.createAuthorizationError(permissionResult.message || 'Unauthorized to create agent group');
      }
  ...
  async updateAgentGroup(request: UpdateAgentGroupRequest): ServiceResult<void> {
    ...
    try {
      ...
      if (!permissionResult.isPermitted) {
        throw this.createAuthorizationError(permissionResult.message || 'Unauthorized to update agent group');
      }
  ...
  async deleteAgentGroup(request: DeleteAgentGroupRequest): ServiceResult<void> {
    ...
    try {
      ...
      if (!permissionResult.isPermitted) {
        throw this.createAuthorizationError(permissionResult.message || 'Unauthorized to delete agent group');
      }
