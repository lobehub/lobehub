...
export class FileUploadService extends BaseService {
  ...
  private async assertOwnedKnowledgeBase(
    ...
    if (!permissionResult.isPermitted) {
      throw this.createAuthorizationError(permissionResult.message || 'Unauthorized to access knowledge base files');
    }
    ...
    if (!knowledgeBase) {
      throw this.createNotFoundError('Knowledge base does not exist or access denied');
    }
  ...
  async uploadFiles(request: BatchFileUploadRequest): Promise<BatchFileUploadResponse> {
    try {
      ...
      if (!isPermitted.isPermitted) {
        throw this.createAuthorizationError(isPermitted.message || 'Unauthorized to upload files');
      }
  ...
  async getFileList(request: FileListQuery): Promise<FileListResponse> {
    try {
      ...
      if (!permissionResult.isPermitted) {
        throw this.createAuthorizationError(permissionResult.message || 'Unauthorized to access file list');
      }
  ...
  async getKnowledgeBaseFileList(
    ...
    try {
      ...
      if (!permissionResult.isPermitted) {
        throw this.createAuthorizationError(permissionResult.message || 'Unauthorized to access knowledge base file list');
      }
  ...
  async addFilesToKnowledgeBase(
    ...
    try {
      ...
      const failed = uniqueFileIds
        ...
        .map((fileId) => ({ fileId, reason: 'File does not exist or access denied' }));
  ...
  async getFileDetail(fileId: string): Promise<FileDetailResponse> {
    try {
      ...
      if (!permissionResult.isPermitted) {
        throw this.createAuthorizationError(permissionResult.message || 'Unauthorized to access this file');
      }
  ...
  async createChunkTask(
    ...
    try {
      ...
      if (!permissionResult.isPermitted) {
        throw this.createAuthorizationError(permissionResult.message || 'Unauthorized to operate on this file');
      }
  ...
  async deleteFile(fileId: string): Promise<void> {
    try {
      ...
      if (!permissionResult.isPermitted) {
        throw this.createAuthorizationError(permissionResult.message || 'Unauthorized to delete this file');
      }
  ...
  private async findExistingUserFile(hash: string): Promise<FileItem | null> {
    try {
      ...
      this.handleServiceError(error, 'find existing file record by hash for user');
    }
  }
  ...
  async updateFile(
    ...
    try {
      ...
      if (!permissionResult.isPermitted) {
        throw this.createAuthorizationError(permissionResult.message || 'Unauthorized to update file');
      }
