...
describe('MessageModel Update Tests', () => {
  ...
  describe('deleteAllMessages', () => {
    ...
    // A share visitor's conversation now belongs to the visitor's own userId
    // (topics.shareId is the provenance marker), so a creator-scoped
    // `deleteAllMessages()` can no longer reach a visitor's topic or messages
    // at all — there is nothing left to snapshot or interrupt. See the
    // `MessageModel` constructor's JSDoc and `TopicModel` / `AgentModel`'s
    // `onShareRunsInterrupted` split for where that responsibility now lives.
