...
export interface SearchReindexEntityProgress {
  ...
  processedCount: number;
  status: SearchReindexEntityStatus;
}

export interface SearchReindexFailure {
  attempts: number;
  documentId: string;
  entity: SearchDocumentEntity;
  error: string;
...
export interface SearchReindexRun {
  ...
  status: SearchReindexRunStatus;
  updatedAt: string;
}

export interface SearchReindexRunState {
  progress: SearchReindexEntityProgress[];
...
const entitySchema = z.enum(SEARCH_DOCUMENT_ENTITIES);
...
const checkpointSchema = z
  .object({
    ...
    progress: z.array(progressSchema).length(SEARCH_DOCUMENT_ENTITIES.length),
    run: runSchema,
  })
  .superRefine((checkpoint, context) => {
    const entities = new Set(checkpoint.progress.map(({ entity }) => entity));
    if (entities.size !== SEARCH_DOCUMENT_ENTITIES.length) {
      context.addIssue({
        code: 'custom',
        message: 'Search reindex checkpoint must contain each entity exactly once',
        path: ['progress'],
      });
    }
    for (const progress of checkpoint.progress) {
      const expectedIndex = getSearchPhysicalIndexName(
...
type SearchReindexCheckpointFile = z.infer<typeof checkpointSchema>;
...
const stateOf = ({ progress, run }: SearchReindexCheckpointFile): SearchReindexRunState => ({
  progress,
  run,
});
...
export class SearchReindexFileRepository {
  private readonly runPaths = new Map<string, string>();
  private readonly stateDirectory: string;

  constructor(private readonly options: SearchReindexFileRepositoryOptions) {
  ...
  private async findCheckpointPath(runId: string): Promise<string | undefined> {
    ...
    if (checkpoint.run.id !== runId) {
      throw new Error(`Search reindex checkpoint run ID changed unexpectedly: ${checkpointPath}`);
    }
  ...
  private async readCheckpointIfExists(
    checkpointPath: string,
  ): Promise<SearchReindexCheckpointFile | undefined> {
    let source: string;
    try {
    ...
    try {
      ...
      throw new Error(`Search reindex checkpoint is not valid JSON: ${checkpointPath}`, {
        cause: error,
      });
    }
    const parsed = checkpointSchema.safeParse(json);
    if (!parsed.success) {
      throw new Error(`Search reindex checkpoint is invalid: ${checkpointPath}`, {
        cause: parsed.error,
      });
    }
    return parsed.data;
  ...
  private async readCheckpoint(checkpointPath: string): Promise<SearchReindexCheckpointFile> {
  ...
  private async updateCheckpoint<Result>(
    runId: string,
    operation: (checkpoint: SearchReindexCheckpointFile) => Result,
  ): Promise<Result> {
  ...
  async getTargetRun(
    namespace: string,
    schemaVersion: number,
  ): Promise<SearchReindexRunState | undefined> {
    const checkpointPath = this.checkpointPath(namespace, schemaVersion);
    const checkpoint = await this.readCheckpointIfExists(checkpointPath);
