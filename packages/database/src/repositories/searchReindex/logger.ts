...
import type { SearchReindexProgressEvent } from './service';

type JsonPrimitive = boolean | null | number | string;
export type SearchReindexAuditValue =
  JsonPrimitive | SearchReindexAuditValue[] | { [key: string]: SearchReindexAuditValue };

export interface SearchReindexAuditEvent {
  [key: string]: SearchReindexAuditValue;
  type: string;
}

export interface SearchReindexFileLoggerOptions {
  runId: string;
  sessionId: string;
...
export const summarizeSearchReindexError = (error: unknown) => {
...
const assertCredentialSafe = (value: unknown, fieldPath = 'event'): void => {
  ...
  for (const [key, child] of Object.entries(value)) {
    if (FORBIDDEN_FIELD_PATTERN.test(key)) {
      throw new Error(
        `Search reindex audit log refuses credential-shaped field ${fieldPath}.${key}`,
      );
    }
...
export class SearchReindexFileLogger {
  readonly eventsPath: string;
  readonly runDirectory: string;
  readonly summaryPath: string;
  private writeQueue: Promise<void> = Promise.resolve();

  constructor(private readonly options: SearchReindexFileLoggerOptions) {
  ...
  append(event: SearchReindexAuditEvent | SearchReindexProgressEvent) {
  ...
  writeSummary<Summary extends { [key: string]: SearchReindexAuditValue }>(summary: Summary) {
