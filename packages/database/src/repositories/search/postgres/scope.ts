...
import type { SearchBackendScope } from '../types';

/** Columns shared by the workspace-aware tables searched by pg_search. */
export interface WorkspaceScopedColumns {
  userId: AnyPgColumn;
  visibility?: AnyPgColumn;
  workspaceId: AnyPgColumn;
}

/** Shared state and query-shaping helpers used by the PostgreSQL search modules. */
export interface PostgresSearchContext {
  db: LobeChatDatabase;
  ...
  scanScopeWhere: (cols: WorkspaceScopedColumns) => SQL;
...
export function createPostgresSearchContext(
  db: LobeChatDatabase,
  scope: SearchBackendScope,
): PostgresSearchContext {
  ...
  const normalizedScope: SearchBackendScope = {
