...
-- NOTE:
--   Neon and some managed Postgres providers no longer allow pg_search/bm25.
--   This migration is performance-only, so skip safely when bm25 is unavailable.
DO $$
BEGIN
  ...
  ELSE
    RAISE NOTICE 'Skipping bm25 indexes: access method bm25 not available.';
  END IF;
END $$;
