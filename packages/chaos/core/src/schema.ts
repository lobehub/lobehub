...
export const chaosEffectSchema = z.discriminatedUnion('type', [
  z.object({ durationMs: z.number().int().nonnegative(), type: z.literal('delay') }),
  z.object({ count: z.number().int().min(2), type: z.literal('duplicate') }),
  ...
  z.object({ content: z.string(), type: z.literal('replace_result') }),
  z.object({
...
export const chaosExperimentSchema = z.object({
  ...
  target: z.object({ adapter: z.string().min(1), selector: recordSchema }),
