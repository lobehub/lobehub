...
export interface AgentManagementContext {
   ...
   * searchAgent first. The current/responding agent (see `currentAgent`) is NEVER
   * included in this list, so the model cannot accidentally delegate to itself.
   */
  ...
  /**
   * The agent that is currently responding (i.e. "you", from the model's perspective).
   * Injected so the model knows its own identity in multi-agent scenarios and to
   * make explicit that it should not delegate calls back to itself.
   */
  currentAgent?: AvailableAgentInfo;
...
const defaultFormatContext = (context: AgentManagementContext): string => {
  ...
  // Add current_agent section (the agent that is responding — i.e. "you")
  if (context.currentAgent) {
    const { id, title, description } = context.currentAgent;
    const descLine = description ? `\n  <description>${escapeXml(description)}</description>` : '';
    parts.push(
      `<current_agent>\n  <id>${escapeXml(id)}</id>\n  <title>${escapeXml(title)}</title>${descLine}\n</current_agent>`,
    );
  }
  ...
  const hasCurrentAgent = !!context.currentAgent;

  const instructionParts: string[] = [];
  if (hasCurrentAgent) {
    instructionParts.push(
      'You are the agent described in the `current_agent` block above. Treat that as your own identity. Never call yourself via the Agent Management `callAgent` tool — you are not in the `available_agents` list.',
    );
  }
