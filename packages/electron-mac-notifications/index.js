...
// The relative .node require only resolves when this package stays external to
// the main-process bundle (native-deps machinery); bundling it would break the
// path and the asar-unpack redirect.
let binding = null;
if (process.platform === 'darwin') {
  try {
    binding = require('./build/Release/lobehub_mac_notifications.node');
  } catch (error) {
    console.error('[electron-mac-notifications] failed to load native binding:', error.message);
  }
}
