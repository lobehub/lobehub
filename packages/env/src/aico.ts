...
declare global {
  ...
  namespace NodeJS {
    interface ProcessEnv {
      /**
       * When `1`, mock top-up is allowed in non-production only.
       * Ignored / ineffective when `NODE_ENV=production` (always forbidden).
       */
...
export const tomanToUsd = (amountToman: number, rate = aicoEnv.AICO_TOMAN_PER_USD): number => {
  ...
  if (!Number.isFinite(rate) || rate <= 0 || !Number.isInteger(rate)) throw new Error('Invalid FX rate');
