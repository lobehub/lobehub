    ...
    if (isArgumentsStreaming && !id) {
      return (
        <div className={cx(inspectorTextStyles.root, shinyTextStyles.shinyText)}>
          <span>{t('builtins.lobe-personal-pages.apiName.modifyNodes')}</span>
        </div>
      );
    }
          ...
          <>
            ...
            <span className={styles.subdued}>
              {typeof successCount === 'number' && typeof totalCount === 'number'
                ? t('builtins.lobe-personal-pages.inspector.opsResult', {
                    success: successCount,
                    total: totalCount,
                  })
                : t('builtins.lobe-personal-pages.inspector.opsCount', { count: opCount })}
            </span>
          </>
        )}
      </div>
