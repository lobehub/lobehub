...
describe('search reindex metrics', () => {
  beforeEach(() => {
  ...
  it('records batch and durable checkpoint counts with bounded labels', () => {
    ...
    const documents = mocks.counters.get('search_reindex_documents_total');
    ...
    expect(mocks.gauges.get('search_reindex_checkpoint_documents')?.record).toHaveBeenCalledWith(
      10,
      { entity: 'messages', result: 'scanned' },
    );
  });
  ...
  it('records signed reconciliation drift without run or document identifiers', () => {
    ...
    expect(mocks.gauges.get('search_reindex_reconciliation_drift')?.record).toHaveBeenCalledWith(
      2,
      { entity: 'messages' },
    );
    expect(mocks.counters.get('search_reindex_reconciliations_total')?.add).toHaveBeenCalledWith(
      1,
      {
        entity: 'messages',
        result: 'drift',
  ...
  it('records bulk retries without status or error labels', () => {
    ...
    expect(mocks.counters.get('search_reindex_bulk_retries_total')?.add).toHaveBeenCalledWith(1, {
      entity: 'messages',
    });
  });
  ...
  it('records each completed bulk request for batch-size and concurrency tuning', () => {
    ...
    expect(mocks.counters.get('search_reindex_bulk_requests_total')?.add).toHaveBeenCalledWith(
      2,
      attributes,
    );
    expect(mocks.counters.get('search_reindex_bulk_bytes_total')?.add).toHaveBeenCalledWith(
      8192,
      attributes,
    );
    expect(
      mocks.histograms.get('search_reindex_bulk_request_attempts')?.record,
    ).toHaveBeenCalledWith(2, attributes);
    expect(mocks.histograms.get('search_reindex_bulk_request_size')?.record).toHaveBeenCalledWith(
      4096,
      attributes,
    );
    expect(
      mocks.histograms.get('search_reindex_bulk_request_duration')?.record,
    ).toHaveBeenCalledWith(750, attributes);
    expect(mocks.histograms.get('search_reindex_bulk_request_items')?.record).toHaveBeenCalledWith(
      100,
      attributes,
    );
  });
  ...
  it('does not expose metric recorder failures to the reindex operation', () => {
    recordSearchReindexBulkRetry('messages');
    const add = mocks.counters.get('search_reindex_bulk_retries_total')!.add;
    add.mockClear();
