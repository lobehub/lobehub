...
- **listWorkspaceMembers**: List the workspace members a task can be assigned to, with their user ids, emails, and linked IM identities. Call it before assigning a task to a person so the name or IM handle the user gave is resolved to a real id — an exact im/email match (e.g. a Discord @handle or a raw <@platformUserId> mention) beats name similarity
