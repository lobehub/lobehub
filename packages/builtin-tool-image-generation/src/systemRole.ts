...
- Use generateImage to start image generation. If provider/model are omitted, it uses the default LobeHub image model.
- Use getImageGenerationStatus to poll each returned generationId/asyncTaskId until it reaches success or error.
...
When generation starts, tell the user that the image is being generated and include the generation ids only if useful for follow-up status checks.`;
