...
/**
 * Excel's General format shows a number longer than 11 digits in scientific
 * notation, and the 'xlsx' library formats it the same way, so an order number
 * or phone number such as 8613812345678 is read as "8.61381E+12" with its digits
 * gone. A whole number in General format is written in full instead; a format
 * the author set keeps its formatted text.
 */
function keepWholeNumberDigits(worksheet: xlsx.WorkSheet): void {
...
export class ExcelLoader implements FileLoaderInterface {
  async loadPages(filePath: string): Promise<DocumentPage[]> {
    ...
    try {
      ...
      // cellNF keeps each cell's number format, which tells General apart from a format the author set.
      const workbook = xlsx.read(dataBuffer, { cellNF: true, type: 'buffer' });
