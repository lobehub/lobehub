...
describe('isAutoGeneratedImageModelId', () => {
  it('should identify any synthesized :image model id', () => {
    ...
    expect(isAutoGeneratedImageModelId('openai/gpt-5-image:image')).toBe(true);
  });

  it('should reject base model ids without the :image suffix', () => {
    expect(isAutoGeneratedImageModelId('gemini-3.1-flash-image-preview')).toBe(false);
    expect(isAutoGeneratedImageModelId('openai/gpt-5-image')).toBe(false);
  });
});
...
describe('postProcessModelList', () => {
  ...
  it('should not duplicate image clones when run twice', async () => {
    const once = await postProcessModelList(mockModels);
    const twice = await postProcessModelList(once);

    const imageIds = twice.filter((m) => m.id.endsWith(':image')).map((m) => m.id);
    expect(imageIds).toEqual(['gemini-2.5-flash-image-preview:image']);
    expect(twice.length).toBe(once.length);
  });

  it('should synthesize :image clones from imageOutput without whitelist', async () => {
    const result = await postProcessModelList([
      {
    ...
    expect(result.find((m) => m.id === 'openai/gpt-5-image:image')).toMatchObject({
      type: 'image',
    });
    // Routers are not dedicated generators — no clone
    expect(result.find((m) => m.id === 'openrouter/auto:image')).toBeUndefined();
  });
});
