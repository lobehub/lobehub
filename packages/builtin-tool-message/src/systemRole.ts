...
- \`readMessages\` is designed for **small, targeted reads** (up to 100 messages per call). For quick context (e.g. "what was just discussed", "summarize the last few messages"), just call \`readMessages\` with \`channelId\` and \`platform\`. The \`before\`/\`after\` parameters are **optional** — only provide them when you have a specific message ID to paginate from. Do NOT pass empty strings for these fields; omit them entirely when not needed.
