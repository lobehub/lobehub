...
- \`readMessages\` parameters:
  - \`channelId\` and \`platform\` are **required**. All other parameters are **optional** — omit them when not needed.
  - **\`limit\`: NEVER pass this parameter unless the user explicitly specifies a number** (e.g. "show me the last 50 messages"). Each tool call has a high fixed overhead (~20k tokens), so fetching too few messages (e.g. 10) is extremely wasteful. The default value is already set to maximize efficiency per call — do NOT override it with a small number. When in doubt, omit \`limit\`.
  - \`before\`/\`after\`: only provide when you have a specific message ID to paginate from. Do NOT pass empty strings — omit entirely.
  - For quick context (e.g. "what was just discussed", "summarize the last few messages"), just call \`readMessages\` with only \`channelId\` and \`platform\` — no other parameters needed.
