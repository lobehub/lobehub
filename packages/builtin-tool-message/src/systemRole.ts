...
- \`readMessages\` is designed for **small, targeted reads**. Do NOT pass \`limit\` unless the user explicitly asks for a specific number — omit it to use the default. The per-call maximum varies by platform. For quick context (e.g. "what was just discussed", "summarize the last few messages"), just call \`readMessages\` with \`channelId\` and \`platform\`.
