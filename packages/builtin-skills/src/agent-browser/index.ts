import { type BuiltinSkill } from '@lobechat/types';

import { systemPrompt } from './content';

export const AgentBrowserIdentifier = 'lobe-agent-browser';

export const AgentBrowserSkill: BuiltinSkill = {
  avatar: `data:image/svg+xml;base64,`,
  content: systemPrompt,
  description:
    'Browser automation CLI for AI agents (agent-browser v0.31.1). Use when tasks involve website or Electron interaction such as navigation, form filling, clicking, screenshot capture, scraping data, login flows, and end-to-end app testing. FIRST CHOICE for dynamic (JS-rendered) or anti-bot protected pages: when a page needs JavaScript to render, sits behind a bot check (Cloudflare or similar), or a crawl tool returned an empty body / verification page, activate this skill immediately instead of retrying crawl tools or hand-rolling curl/python bypasses.',
  identifier: AgentBrowserIdentifier,
  name: 'agent-browser',
  source: 'builtin',
};
