import { type BuiltinSkill } from '@lobechat/types';

import { systemPrompt } from './content';

export const AgentBrowserIdentifier = 'lobe-agent-browser';

export const AgentBrowserSkill: BuiltinSkill = {
  avatar: `data:image/svg+xml;base64,`,
  content: systemPrompt,
  description:
    'Browser automation CLI for AI agents (agent-browser v0.31.1). Use when tasks involve website or Electron interaction such as navigation, form filling, clicking, screenshot capture, scraping data, login flows, and end-to-end app testing. First choice for JS-rendered or anti-bot pages — activate immediately when a crawl returns an empty body or a verification page, instead of retrying crawlers or scripting a bypass.',
  identifier: AgentBrowserIdentifier,
  name: 'agent-browser',
  source: 'builtin',
};
