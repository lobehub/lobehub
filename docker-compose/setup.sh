...
show_message() {
  ...
  case $key in
    ...
    tips_already_installed)
      case $LANGUAGE in
        zh_CN)
          ...
          ;;
        *)
          echo "It is detected that you have run ChinnaHub. This installation program can only complete the initialization configuration and cannot be reinstalled. If you need to reinstall, please delete the data and s3_data folders."
          ;;
      esac
      ;;
    tips_run_command)
      case $LANGUAGE in
        zh_CN)
          echo "您已经完成了所有配置。请运行以下命令启动 ChinnaHub 尝试启动："
          ;;
        *)
          echo "You have completed all configurations. Please run this command to start ChinnaHub:"
          ;;
      esac
      ;;
    ...
    tips_if_run_normally)
      case $LANGUAGE in
        zh_CN)
          echo "如果一切运行正常，你可以使用以下指令在 daemon 模式下启动 ChinnaHub:"
          ;;
        *)
          echo "If everything runs normally, you can use the following command to start ChinnaHub in daemon mode:"
          ;;
      esac
      ;;
    tips_regen_jwks)
